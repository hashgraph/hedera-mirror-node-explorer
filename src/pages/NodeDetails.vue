        ...
        <DashboardCard>
            <template v-slot:title>
                <div class="is-flex is-align-items-center">
                    ...
                    <span class="h-is-secondary-text is-numeric mr-3">{{
                        nodeIdNb
                    }}</span>
                </div>
                <div v-if="isCouncilNode">
                    ...
                    ></span>
                    <span class="h-is-tertiary-text has-text-grey"
                        >Hedera Council Node</span
                    >
                </div>
                <div v-else>
                    <span class="icon has-text-info mr-2"
            ...
            <template v-slot:leftContent>
                ...
                <Property id="serviceEndpoints">
                    ...
                    <template v-slot:value>
                        <Endpoints
                            :endpoints="node?.service_endpoints"
                        ></Endpoints>
                    </template>
                </Property>
            ...
            <template v-slot:rightContent>
                ...
                <p
                    v-if="stake"
                    id="consensusStakePercent"
                    class="h-is-property-text h-is-extra-text mt-1"
                    >{{ stakePercentage }} of total</p
                >
                <p v-else class="h-is-property-text h-is-extra-text mt-1"
                    >(&lt;Min)</p
                >
                <br /><br />
                <div v-if="stake === 0">
                    <NetworkDashboardItem
                        id="currentStake"
                        :value="makeFloorHbarAmount(unclampedStake)"
                        name="HBAR"
                        title="Current Stake"
                    />
                    <br /><br />
                ...
                <p
                    id="rewardedPercent"
                    class="h-is-property-text h-is-extra-text mt-1"
                    >{{ stakeRewardedPercentage }}% of total</p
                >
                <br /><br />
                <NetworkDashboardItem
                    id="notRewarded"
                ...
                />
                <p
                    id="notRewardedPercent"
                    class="h-is-property-text h-is-extra-text mt-1"
                    >{{ stakeUnrewardedPercentage }}% of total</p
                >
                <br /><br />
                <NetworkDashboardItem
                    id="stakingPeriod"
                ...
                <p class="h-is-property-text h-is-extra-text mt-1"
                    >from 00:00 am today to 11:59 pm today UTC</p
                >
                <div class="mt-6" />
                <br />
            </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const stakeTotal = computed(
            () => stakeLookup.entity.value?.stake_total ?? 0,
        );
        const stakePercentage = computed(() =>
            nodeAnalyzer.node.value && stakeTotal.value
                ? makeStakePercentage(
                      nodeAnalyzer.node.value as NetworkNode,
                      stakeTotal.value,
                  )
                : "0",
        );
        ...
        const stakeRewardedPercentage = computed(() =>
            networkAnalyzer.stakeRewardedTotal.value != 0
                ? Math.round(
                      (nodeAnalyzer.stakeRewarded.value /
                          networkAnalyzer.stakeRewardedTotal.value) *
                          10000,
                  ) / 100
                : 0,
        );
        ...
        const stakeUnrewardedPercentage = computed(() =>
            networkAnalyzer.stakeUnrewardedTotal.value != 0
                ? Math.round(
                      (nodeAnalyzer.stakeUnrewarded.value /
                          networkAnalyzer.stakeUnrewardedTotal.value) *
                          10000,
                  ) / 100
                : 0,
        );
        ...
        const makeFloorHbarAmount = (tinyBarAmount: number) =>
            Math.floor((tinyBarAmount ?? 0) / 100000000).toLocaleString(
                "en-US",
            );
