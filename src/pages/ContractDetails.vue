...
<template>
    ...
    >
        <DashboardCard>
            ...
            <template v-slot:leftContent>
                <Property id="balance">
                    <template v-slot:name>{{
                        tokens?.length ? "Balances" : "Balance"
                    }}</template>
                ...
                <Property id="key">
                    ...
                    <template v-slot:value>
                        <KeyValue
                            :key-bytes="contract?.admin_key?.key"
                            :key-type="contract?.admin_key?._type"
                            :show-none="true"
                        />
                    </template>
                </Property>
                ...
                <Property id="createTransaction">
                    ...
                    <template v-slot:value>
                        <TransactionLink
                            :transactionLoc="
                                contract?.created_timestamp ?? undefined
                            "
                        />
                    </template>
                </Property>
                <Property id="expiresAt">
                    <template v-slot:name>
                        ...
                        <InfoTooltip
                            label="Contract expiry is not turned on yet. Value in this field is not relevant."
                        />
                    </template>
                    <template v-slot:value>
                        <TimestampValue
                            v-bind:timestamp="contract?.expiration_timestamp"
                ...
                <Property id="autoRenewPeriod">
                    <template v-slot:name>
                        ...
                        <InfoTooltip
                            label="Contract auto-renew is not turned on yet. Value in this field is not relevant."
                        />
                    </template>
                    <template v-slot:value>
                        <DurationValue
                            v-bind:number-value="
                                contract?.auto_renew_period ?? undefined
                            "
                        />
                    </template>
                </Property>
                ...
                <Property id="maxAutoAssociation">
                    ...
                    <template v-slot:value>
                        <StringValue
                            :string-value="
                                contract?.max_automatic_token_associations?.toString()
                            "
                        />
                    </template>
                </Property>
            ...
            <template v-slot:rightContent>
                ...
                <Property id="validFrom">
                    ...
                    <template v-slot:value>
                        <TimestampValue
                            :timestamp="contract?.timestamp?.from"
                            :show-none="true"
                        />
                    </template>
                </Property>
                <Property id="validUntil">
                    ...
                    <template v-slot:value>
                        <TimestampValue
                            :timestamp="contract?.timestamp?.to"
                            :show-none="true"
                        />
                    </template>
                </Property>
        ...
        <ContractResultsSection
            :contract-id="normalizedContractId ?? undefined"
        />
    </section>
    ...
    setup(props) {
        ...
        const validEntityId = computed(() => {
            return props.contractId
                ? EntityID.parse(props.contractId, true) != null
                : false;
        });
        const normalizedContractId = computed(() => {
            return props.contractId
                ? EntityID.normalize(props.contractId)
                : null;
        });
        ...
        const contractLookup =
            ContractByIdCache.instance.makeLookup(normalizedContractId);
        ...
        const displayNonce = computed(
            () => contractLookup.entity.value?.nonce != undefined,
        );
        ...
        const displayAllTokenLinks = computed(() =>
            accountLocParser.tokens.value
                ? accountLocParser.tokens.value.length > MAX_TOKEN_BALANCES
                : false,
        );
        ...
        const accountRoute = computed(() => {
            return normalizedContractId.value !== null
                ? routeManager.makeRouteToAccount(normalizedContractId.value)
                : null;
        });
