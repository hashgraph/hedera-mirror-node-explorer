        ...
        <DashboardCard>
            <template v-slot:title>
                <span class="h-is-primary-title"
                    >Block {{ block?.number?.toString() ?? "" }}</span
                >
            </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        // block
        //
        const blockLocParser = new BlockLocParser(
            computed(() => props.blockHon ?? null),
        );
        ...
        // transactions
        //
        const transactionsLookup =
            TransactionGroupByBlockCache.instance.makeLookup(
                blockLocParser.blockNumber,
            );
        onMounted(() => transactionsLookup.mount());
        onBeforeUnmount(() => transactionsLookup.unmount());
        const transactions = computed(
            () => transactionsLookup.entity.value ?? [],
        );

        const disablePreviousButton = ref(true);
        const disableNextButton = ref(true);
        watch(blockLocParser.block, () => {
            disablePreviousButton.value =
                blockLocParser.errorNotification.value != null ||
                blockLocParser.block.value?.previous_hash === nullHash;
