...
<template>
    <StakingDialog
        v-model:show-dialog="showStakingDialog"
    ...
    <ConfirmDialog
        v-model:show-dialog="showStopConfirmDialog"
        @onConfirm="handleStopStaking"
        :main-message="'Do you want to stop staking to ' + stakedTo + '?'"
    >
        <template v-slot:dialogTitle>
            <span class="h-is-primary-title">My Staking </span>
            <span v-if="accountId" class="h-is-tertiary-text">
                for account
            </span>
            <span
                v-if="accountId"
    ...
    >
        <DashboardCard>
            <template v-slot:title>
                <div>
                    <span class="h-is-primary-title">My Staking </span>
                    <span v-if="accountId" class="h-is-primary-title">
                        for account
                    </span>
                    <div
                        v-if="accountId"
                ...
                >
                    ...
                    <router-link
                        v-if="allowanceApprovalRoute"
                        :to="allowanceApprovalRoute"
                    >
                        <span class="h-is-property-text"
                            >Approve an allowance…</span
                        >
                    </router-link>
                </div>
            </template>
            ...
            <template v-slot:control v-if="isMediumScreen">
                <div
                    v-if="accountId"
                    id="showAccountLink"
                    class="is-flex is-flex-direction-column ml-3"
                >
                    <router-link v-if="accountRoute" :to="accountRoute">
                        <span class="h-is-property-text">Show my account</span>
                    </router-link>
            ...
            <template v-slot:content>
                <template v-if="accountId">
                    <div v-if="isSmallScreen">
                        <div class="is-flex is-justify-content-space-between">
                            ...
                            <NetworkDashboardItem
                                ...
                                :class="{
                                    'h-has-opacity-40':
                                        ignoreReward && !pendingReward,
                                }"
                            />
                        </div>
                        <div
                            class="is-flex is-justify-content-space-between mt-5"
                        >
                            ...
                            <button
                                ...
                                >DISCONNECT
                                {{ walletName.toLocaleUpperCase() }}</button
                            >
                        </div>
                        <div
                            class="mt-5 h-is-text-size-2 is-italic has-text-grey has-text-centered"
                        >
                            <span class="has-text-grey-light"
                                >Please Note:
                            </span>
                            Your full balance is automatically staked.<br />
                            Your funds are fully available for use while
                            staked.<br />
                    ...
                    <div v-else>
                        ...
                        <div class="is-flex is-justify-content-center">
                            ...
                            >
                            <button
                                id="showStakingDialogSmall"
                                class="button is-white is-small ml-4"
                                @click="showStakingDialog = true"
                                >CHANGE STAKED TO</button
                            >
                        </div>
                        <div class="is-flex is-justify-content-center mt-4">
                            <button
                                id="disconnectWalletButtonSmall"
                                class="button is-white is-small"
                                @click="disconnectFromWallet"
                ...
                <template v-else-if="connecting">
                    ...
                    >
                        ...
                        <p
                            >You need to select which account you wish to
                            connect.</p
                        >
                        <br />
                        <button
                            id="abortConnectWalletButton"
                            class="button is-white is-small"
                            @click="disconnectFromWallet"
                ...
                <template v-else>
                    <section
                        class="section has-text-centered pt-0"
                        :class="{ 'pb-0': isSmallScreen }"
                    >
                        <p class="h-is-tertiary-text" style="font-weight: 300">
                            To view or change your staking options first connect
                            your wallet.
                        </p>
                        <br />
                        <button
        ...
        <DashboardCard
            v-if="accountId"
            :class="{ 'h-has-opacity-40': !isStakedToNode }"
        >
            <template v-slot:title>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const isStakedToNode = computed(
            () => accountLocParser.stakedNodeId.value !== null,
        );
        const isStakedToAccount = computed(
            () => accountLocParser.stakedAccountId.value,
        );
        const isStaked = computed(
            () => isStakedToNode.value || isStakedToAccount.value,
        );
        ...
        const stakedTo = computed(() => {
            ...
            if (isStakedToAccount.value) {
                ...
                result =
                    "Node " +
                    accountLocParser.stakedNodeId.value +
                    " - " +
                    stakedNodeAnalyzer.shortNodeDescription.value;
            } else {
                result = null;
            }
            return result;
        ...
        const accountRoute = computed(() => {
            return walletManager.accountId.value !== null
                ? routeManager.makeRouteToAccount(
        ...
        const stakedAmount = computed(() =>
            isStaked.value
                ? formatHbarAmount(accountLocParser.balance.value)
                : null,
        );
        ...
        const pendingReward = computed(() =>
            formatHbarAmount(accountLocParser.pendingReward.value ?? null),
        );
        const declineReward = computed(
            () => accountLocParser.accountInfo.value?.decline_reward ?? false,
        );
        const ignoreReward = computed(
            () => accountLocParser.stakedNodeId.value === null,
        );
        ...
        const stakedNodeAnalyzer = new NodeAnalyzer(
            accountLocParser.stakedNodeId,
        );
        ...
        const handleStopStaking = () => {
            changeStaking(
                null,
                null,
                accountLocParser.accountInfo.value?.decline_reward
                    ? false
                    : null,
            );
        };

        const handleChangeStaking = (
            nodeId: number | null,
        ...
        const changeStaking = async (
            ...
            try {
                ...
                progressDialogTitle.value =
                    nodeId == null && accountId == null && !declineReward
                        ? "Stopping staking"
                        : "Updating staking";
                ...
                const transactionId = normalizeTransactionId(
                    await walletManager.changeStaking(
                        nodeId,
                        accountId,
                        declineReward,
                    ),
        ...
        const waitForTransactionRefresh = async (transactionId: string) => {
            ...
            try {
                ...
                while (counter > 0 && transaction === null) {
                    ...
                    transaction = await TransactionByIdCache.instance.lookup(
                        transactionId,
                        true,
                    );
                    counter -= 1;
