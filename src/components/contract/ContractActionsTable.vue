        ...
        >
            <o-table-column v-slot="props" field="call_type" label="Call Type">
                <div class="is-flex is-align-items-baseline">
                    <span
                        class="is-family-monospace h-is-text-size-3 has-text-grey"
                    >
                        {{ props.row.depthPath }}
                    </span>
                    <span
                        v-if="isSuccessful(props.row.action)"
                        class="ml-2 h-has-pill h-is-text-size-1 has-background-success"
                    >
                        {{ makeOperationType(props.row.action) }}
                    </span>
                    <span
                        v-else
                        class="ml-2 h-has-pill h-is-text-size-2 has-background-danger"
                    >
                        {{ "! " + makeOperationType(props.row.action) }}
                    </span>
                </div>
            ...
            <o-table-column v-slot="props" field="amount" label="Amount">
                <div
                    class="is-flex is-align-items-end is-align-content-end is-numeric"
                >
                    <span style="font-size: 13px; margin-right: 2px"
                        >&#8594;</span
                    >
                    <HbarAmount
                        ...
                        :show-extra="true"
                    />
                    <span style="font-size: 13px; margin-left: 2px; :2px"
                        >&#8594;</span
                    >
                </div>
            </o-table-column>
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const isPaginated = computed(
            () => (props.actions?.length ?? 0) > NB_ACTIONS_PER_PAGE,
        );

        const isSuccessful = (action: ContractAction) =>
            action.result_data_type == "OUTPUT";

        const makeOperationType = (action: ContractAction) =>
            action.call_operation_type ?? action.call_type;
