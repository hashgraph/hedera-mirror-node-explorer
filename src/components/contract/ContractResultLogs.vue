...
export default defineComponent({
    ...
    setup(props) {
        ...
        const pageSize = ref(
            AppStorage.getLogsTablePageSize() ?? DEFAULT_PAGE_SIZE,
        );
        watch(pageSize, () => {
        ...
        const isPaginated = computed(
            () => props.logs?.length && props.logs?.length > pageSize.value,
        );
        const logCursor = computed(
            () => (currentPage.value - 1) * pageSize.value,
        );
        const nbLogDisplayed = computed(() => {
            return props.logs?.length
                ? Math.min(pageSize.value, props.logs?.length - logCursor.value)
                : 0;
        });
