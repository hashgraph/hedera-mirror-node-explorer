...
<template>
    <DashboardCard v-if="stateChanges?.length && isSmallScreen" class="h-card">
        <template v-slot:title>
            <span class="h-is-secondary-title"
                >Contract States Accessed & Changed</span
            >
        </template>
        ...
        <template v-slot:content>
            <div
                class="columns"
                style="margin-bottom: 0; font-size: 11px; font-weight: 700"
            >
            ...
            <hr
                class="h-card-separator"
                style="margin-top: 0; margin-bottom: 20px; height: 1px"
            />
            ...
            <div v-for="s in nbChangeDisplayed" :key="s">
                ...
                <hr
                    class="h-card-separator"
                    style="margin-top: 0; margin-bottom: 12px; height: 0.5px"
                />
            </div>
    ...
    <DashboardCard v-else-if="stateChanges?.length" class="h-card">
        <template v-slot:title>
            <div class="h-is-secondary-title"
                >Contract States Accessed & Changed</div
            >
        </template>
...
export default defineComponent({
    ...
    props: {
        stateChanges: Object as PropType<
            Array<ContractResultStateChange> | undefined
        >,
        timeStamp: String,
    },
    ...
    setup(props) {
        ...
        const pageSize = ref(
            AppStorage.getStatesTablePageSize() ?? DEFAULT_PAGE_SIZE,
        );
        watch(pageSize, () => {
        ...
        const isPaginated = computed(
            () =>
                displayStateChanges?.value.length &&
                displayStateChanges?.value.length > pageSize.value,
        );
        const changeCursor = computed(
            () => (currentPage.value - 1) * pageSize.value,
        );
        watch(
            changeCursor,
            () => (displayStateChanges.value[changeCursor.value].header = true),
        );
        const nbChangeDisplayed = computed(() => {
            return displayStateChanges?.value.length
                ? Math.min(
                      pageSize.value,
                      displayStateChanges?.value.length - changeCursor.value,
                  )
                : 0;
        });

        const transactionLookup = TransactionByTsCache.instance.makeLookup(
        ...
        watch(
            [() => props.stateChanges, transactionLookup.entity],
            () => (displayStateChanges.value = makeDisplayStateChanges()),
        );
        ...
        const makeDisplayStateChanges = () => {
            ...
            if (props.stateChanges) {
                for (const s of props.stateChanges) {
                    ...
                    if (
                        result.length > 0 &&
                        s.address === result[result.length - 1].changes.address
                    ) {
                        newItem.header = false;
                    } else {
                        newItem.balanceChange = lookupTransfer(
                            s.contract_id ?? "",
                        );
                    }

                    newItem.valueChange =
                        newItem.valueReadDecimal && newItem.valueWrittenDecimal
                            ? newItem.valueWrittenDecimal -
                              newItem.valueReadDecimal
                            : null;
