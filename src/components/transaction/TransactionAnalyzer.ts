...
export class TransactionAnalyzer {
    ...
    public readonly transactionType = computed(
        () => this.transaction.value?.name ?? null,
    );

    public readonly entityId = computed(
        () => this.transaction.value?.entity_id ?? null,
    );
    ...
    public readonly hasSucceeded: ComputedRef<boolean> = computed(
        () => this.result.value == "SUCCESS",
    );

    public readonly netAmount: ComputedRef<number> = computed(() =>
        this.transaction.value !== null
            ? computeNetAmount(
                  this.transaction.value?.transfers,
                  this.transaction.value?.charged_tx_fee,
              )
            : 0,
    );

    public readonly maxFee: ComputedRef<number> = computed(() => {
        const result = this.transaction.value?.max_fee
            ? Number.parseFloat(this.transaction.value?.max_fee)
            : 0;
    ...
    public readonly systemContract: ComputedRef<string | null> = computed(
        () => {
            ...
            if (
                this.transaction.value?.name === TransactionType.CONTRACTCALL &&
                this.transaction.value.entity_id
            ) {
                result =
                    systemContractRegistry.lookup(
                        this.transaction.value.entity_id,
                    )?.description ?? null;
            } else {
                result = null;
    ...
    private readonly transactionDidChange = async () => {
        if (this.transaction.value !== null) {
            ...
            if (entityId !== null) {
                switch (this.transaction.value.name) {
                    ...
                    case TransactionType.ETHEREUMTRANSACTION: {
                        const contract =
                            await ContractByIdCache.instance.lookup(entityId);
                ...
                this.entityDescriptor.value =
                    await EntityDescriptor.makeEntityDescriptor(
            ...
            if (consensusTimestamp !== null) {
                const block =
                    await BlockByTsCache.instance.lookup(consensusTimestamp);
