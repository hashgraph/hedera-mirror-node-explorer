...
<!--    </o-table-column>-->
    ...
    <o-table-column v-slot="{ row }" field="balance" label="Balance">
      {{ formatErcBalance(row.balance, 0) }}
    </o-table-column>
...
import {AccountERC721} from "@/utils/cache/AccountERC721Cache.ts";
import {routeManager} from "@/router";
import {formatUnits} from "ethers";

const props = defineProps({
  tokens: {
    type: Object as PropType<AccountERC721[]>,
    required: true
  },
  fullPage: {
    type: Boolean,
...
const handleClick = (row: AccountERC721, c: unknown, i: number, ci: number, event: MouseEvent) => {
  if (row.erc721Info.contractId) {
    routeManager.routeToContract(row.erc721Info.contractId, event)
  }
}
...
const refreshBalance = (row: AccountERC721) => {
  emit("refresh", row)
}
