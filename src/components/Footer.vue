...
<template>
    <section
        ...
        :class="{
            'h-mobile-background':
                !keepBackground && (isTouchDevice || !isSmallScreen),
        }"
    >
        ...
        <div class="is-flex is-align-items-center">
            ...
            <div
                class="is-flex is-flex-direction-column is-align-items-flex-start ml-5"
            >
                <span
                    class="h-is-property-text pb-1"
                    style="font-weight: 300; color: #dbdbdb"
                >
                    <span>{{ productName }}</span>
                    <span v-if="!isTouchDevice && isMediumScreen">
                        is a ledger explorer for the Hedera network.</span
                    >
                </span>
                <span
                ...
                >
                    Release
                    <a :href="buildReleaseUrl">{{ buildRelease }}</a> built
                    {{ buildTime }}
                </span>
                <a
                    data-cy="termsOfUse"
            ...
            <a v-if="sponsorURL" :href="sponsorURL" class="ml-4">
                <img
                    alt="Sponsor Logo"
                    src="@/assets/branding/brand-sponsor-logo.png"
...
export default defineComponent({
    ...
    setup() {
        ...
        const buildShortCommitHash = inject(
            "buildShortCommitHash",
            "not available",
        );
        ...
        const productName =
            import.meta.env.VITE_APP_PRODUCT_NAME ??
            "Hedera Mirror Node Explorer";
        const sponsorURL = import.meta.env.VITE_APP_SPONSOR_URL ?? "";
        const termsOfUseURL = import.meta.env.VITE_APP_TERMS_OF_USE_URL
            ? "/" + import.meta.env.VITE_APP_TERMS_OF_USE_URL
            : "";
