            ...
            <div class="box">
                ...
                >
                    <div v-for="d in drivers" :key="d.name">
                        <a
                            :id="d.name"
                            @click="chosenWallet = d"
                            @dblclick="handleConnect"
                        >
                            <figure
                                :class="{ selected: isSelected(d) }"
                                class="h-chooser-figure my-4 mr-6"
                            >
                                <img
                                    :src="d.iconURL ?? undefined"
                                    alt="wallet logo"
                                    class="h-chooser-img"
                                />
                            </figure>
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const disclaimer =
            import.meta.env.VITE_APP_WALLET_CHOOSER_DISCLAIMER_POPUP ?? "";
        ...
        const isSelected = (wallet: WalletDriver) => {
            return (
                chosenWallet.value && chosenWallet.value.name === wallet.name
            );
        };
