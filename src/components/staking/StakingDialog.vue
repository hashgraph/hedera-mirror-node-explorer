...
<template>
    ...
    <div :class="{ 'is-active': showDialog }" class="modal has-text-white">
        ...
        <div class="modal-content" style="width: 768px; border-radius: 16px">
            <div class="box">
                <div
                    class="is-flex is-justify-content-space-between is-align-items-self-end"
                >
                    <span>
                        <span class="h-is-primary-title">Change Staking </span>
                        <span v-if="accountId" class="h-is-tertiary-text">
                ...
                <Property id="currentlyStakedTo">
                    ...
                    <template v-slot:value>
                        <span
                            ...
                            class="icon is-small has-text-info mr-2"
                            style="font-size: 16px"
                        >
                            <i
                                v-if="currentStakedNodeIcon"
                                :class="currentStakedNodeIcon"
                            ></i>
                        </span>
                        <StringValue
                            v-if="account"
                            :string-value="currentlyStakedTo"
                        />
                    </template>
                </Property>
                            ...
                            <div class="column">
                                <o-field>
                                    <o-select
                                        ...
                                        :class="{
                                            'has-text-grey': !isNodeSelected,
                                        }"
                                        class="h-is-text-size-1"
                                        style="border-radius: 4px"
                                        @focus="stakeChoice = 'node'"
                            ...
                            <div class="column pt-0">
                                <div class="is-flex is-align-items-center">
                                    <input
                                        ...
                                        :class="{
                                            'has-text-grey': !isAccountSelected,
                                        }"
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const confirmMessage = computed(() => {
            ...
            if (isNodeSelected.value) {
                if (selectedNode.value !== props.account?.staked_node_id) {
                    result =
                        "Do you want to stake to Node " +
                        selectedNodeDescription.value +
                        "?";
                } else {
                    result = declineChoice.value
                        ? "Do you want to decline rewards?"
                        : "Do you want to accept rewards?";
                }
            } else {
                result =
                    "Do you want to stake to account " +
                    nr.makeAddressWithChecksum(
                        selectedAccountEntity.value ?? "",
                        network,
                    ) +
                    " ?";
            }
        ...
        const nodeAnalyzer = new NodeAnalyzer(
            computed(() => props.account?.staked_node_id ?? 0),
        );
        ...
        const selectedNodeIcon = computed(() => {
            ...
            if (selectedNode.value !== null) {
                ...
                result = isCouncilNode(nodes.value[selectedNode.value])
                    ? "building"
                    : "users";
            } else {
                result = "";
        ...
        watch(
            accountId,
            () =>
                (declineChoice.value = props.account?.decline_reward ?? false),
        );

        const enableChangeButton = computed(() => {
            return (
                (isAccountSelected.value &&
                    isSelectedAccountValid.value &&
                    props.account?.staked_account_id !=
                        selectedAccountEntity.value) ||
                (isNodeSelected.value &&
                    selectedNode.value !== null &&
                    props.account?.staked_node_id != selectedNode.value) ||
        ...
        const handleConfirmChange = () => {
            ...
            const stakedAccount = isAccountSelected.value
                ? nr.stripChecksum(selectedAccount.value ?? "")
                : null;
            const declineReward =
                declineChoice.value != props.account?.decline_reward
                    ? declineChoice.value
                    : null;
            context.emit(
                "changeStaking",
        ...
        const makeNodeDescription = (node: NetworkNode) => {
            let description =
                node.description ??
                makeDefaultNodeDescription(node.node_id ?? null);
            return description
                ? node.node_id + " - " + makeShortNodeDescription(description)
                : null;
        };
        ...
        const validateAccount = () => {
            if (selectedAccountEntity.value === null) {
                ...
                if (selectedAccountEntity.value == accountId.value) {
                    ...
                    axios
                        .get<AccountsResponse>("api/v1/accounts", {
                            params: params,
                        })
                        .then((response) => {
