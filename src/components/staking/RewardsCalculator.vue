        ...
        <template v-slot:content>
            <div class="columns">
                <div class="column is-three-fifths">
                    ...
                    >
                        <p v-if="isMediumScreen" class="h-is-property-text mb-3"
                            >Choose a node to stake to</p
                        >
                        <p v-else class="h-is-text-size-3 mb-1"
                            >Choose a node to stake to</p
                        >
                ...
                <div class="column">
                    ...
                    >
                        <p v-if="isMediumScreen" class="h-is-property-text mb-3"
                            >Enter the number of hbars you want to stake</p
                        >
                        <p v-else class="h-is-text-size-3 mb-1"
                            >Enter number of hbars you want to stake</p
                        >
                        <input
...
export default defineComponent({
    ...
    setup(props) {
        ...
        watch(
            () => props.nodeId,
            () => (selectedNodeId.value = props.nodeId ?? null),
        );
        ...
        const currentReward = computed(() =>
            rewardRate.value && amountStaked.value
                ? Math.round(amountStaked.value * rewardRate.value * 10000) /
                  10000
                : 0,
        );
        const monthlyReward = computed(() =>
            currentReward.value
                ? Math.round(currentReward.value * 30 * 100) / 100
                : 0,
        );
        const yearlyReward = computed(() =>
            currentReward.value
                ? Math.round(currentReward.value * 365 * 10) / 10
                : 0,
        );
