...
export default defineComponent({
    ...
    setup(props) {
        ...
        const decimalOverflow = computed(() => {
            return decimalCount.value
                ? decimalCount.value > MAX_DECIMALS
                : false;
        });
...
function formatTokenAmount(rawAmount: bigint, decimalCount: number): string {
    ...
    if (decimalCount) {
        result = amountFormatter.format(
            Number(rawAmount) / Math.pow(10, decimalCount),
        );
    } else {
