    ...
    setup(props) {
        ...
        const seconds = computed(() => {
            return props.timestamp !== null
                ? parseSeconds(normalizedTimestamp(props.timestamp, props.nano))
                : null;
        });

        const isNever = computed(
            () => seconds.value && seconds.value >= infiniteDuration,
        );
        ...
        const datePart = computed(() => {
            return seconds.value != null && !isNever.value
                ? dateFormat.format(seconds.value * 1000)
                : "?";
        });

        const timePart = computed(() => {
            return seconds.value && !isNever.value
                ? HMSF.extract(seconds.value, locale)
                : null;
        });
