...
<template>
    ...
    >
        <template v-slot:content>
            <div
                class="is-family-monospace h-is-text-size-3 mt-1"
                :class="{ 'has-text-grey': lowContrast }"
            >
    ...
    <div v-else-if="showNone && !initialLoading">
        ...
        <div v-if="noneExtra" class="has-text-grey h-is-text-size-3">{{
            noneExtra
        }}</div>
    </div>
    <div v-else />
    ...
    setup(props) {
        ...
        const normByteString = computed((): string | undefined => {
            ...
            if (props.byteString !== null) {
                result = props.byteString.startsWith("0x")
                    ? props.byteString.slice(2)
                    : props.byteString;
            } else {
                result = undefined;
            }
        ...
        const flow = (nbWords: number | null): string => {
            return normByteString.value
                ? makeByteLine(normByteString.value, nbWords)
                : "";
        };
