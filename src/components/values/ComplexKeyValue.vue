                ...
                <div :style="lineStyle(line)">
                    <template v-if="line.innerKeyBytes() !== null">
                        <div v-if="details" :class="lineClass(line)">
                            ...
                            <span class="is-family-monospace has-text-grey">{{
                                ":&#8239;" + line.innerKeyBytes()
                            }}</span>
                        </div>
                        <div v-else>
                            <HexaValue :byte-string="line.innerKeyBytes()" />
                            <div class="h-is-extra-text h-is-text-size-3">{{
                                line.innerKeyType()
                            }}</div>
                        </div>
                    </template>
                    ...
                    <template v-else-if="line.delegatableContractId() !== null">
                        ...
                        <ContractLink
                            :contract-id="line.delegatableContractId()"
                        />
                    </template>
                    <template v-else>
                        <div
    ...
    setup(props) {
        const key = computed(() => {
            ...
            if (props.keyBytes) {
                ...
                try {
                    result =
                        keyByteArray !== null
                            ? hashgraph.proto.Key.decode(keyByteArray)
                            : null;
                } catch (reason) {
        ...
        const lines = computed(() => {
            return key.value !== null
                ? ComplexKeyLine.flattenComplexKey(key.value)
                : [];
        });
        ...
        const lineText = (line: ComplexKeyLine): string => {
            ...
            if (line.key.thresholdKey) {
                const childCount =
                    line.key.thresholdKey.keys?.keys?.length ?? 0;
                result =
                    "THRESHOLD (" +
                    line.key.thresholdKey.threshold +
                    " of " +
                    childCount +
                    ")";
