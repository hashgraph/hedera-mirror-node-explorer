...
<template>
    <div v-if="evmAddress">
        <div
            :class="{
                'is-flex': isSmallScreen,
                'h-is-text-size-3': !hasCustomFont,
                'is-family-monospace': !hasCustomFont,
            }"
        ...
        >
            ...
            <span v-if="entityId && showId">
                ...
                <router-link
                    v-if="entityLinkType === CONTRACT"
                    :to="{
                        name: 'ContractDetails',
                        params: { contractId: entityId },
                    }"
                    >{{ entityId }}</router-link
                >
                <router-link
                    v-else-if="entityLinkType === ACCOUNT"
                    :to="{
                        name: 'AccountDetails',
                ...
                >
                <router-link
                    v-else-if="entityLinkType === TOKEN"
                    :to="{
                        name: 'TokenDetails',
                        params: { tokenId: entityId },
                    }"
                    >{{ entityId }}</router-link
                >
        ...
        <div v-if="showType" class="h-is-extra-text h-is-text-size-2">{{
            entityType
        }}</div>
    </div>
...
export enum ExtendedEntityType {
    UNDEFINED,
    ACCOUNT,
    CONTRACT,
    TOKEN,
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const entityLinkType = ref<ExtendedEntityType>(
            ExtendedEntityType.UNDEFINED,
        );
        const evmAddress = ref<string | null>(null);
        const entityId = ref<string | null>(null);
        const ethereumAddress = computed(() =>
            EthereumAddress.parse(evmAddress.value ?? ""),
        );
        const derivedEntityId = computed(
            () => ethereumAddress.value?.toEntityID()?.toString() ?? null,
        );

        onMounted(() => updateIdAndAddress());
        watch(
            [() => props.address, () => props.id, () => props.entityType],
            () => updateIdAndAddress(),
        );
        ...
        const updateIdAndAddress = async () => {
            ...
            if (props.entityType === "ACCOUNT") {
                if (
                    ethereumAddress.value?.isLongZeroForm() ||
                    entityId.value === null
                ) {
        ...
        const updateFromSystemContract = async (): Promise<boolean> => {
            const systemContract = systemContractRegistry.lookup(
                derivedEntityId.value ?? "",
            );
        ...
        const updateFromAccount = async (): Promise<boolean> => {
            const account = await AccountByAddressCache.instance.lookup(
                props.address ?? "",
            );
            if (account !== null) {
        ...
        const updateFromContract = async (): Promise<boolean> => {
            const contract = await ContractByAddressCache.instance.lookup(
                props.address ?? "",
            );
            if (contract !== null) {
