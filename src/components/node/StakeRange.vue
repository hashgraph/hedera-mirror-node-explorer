    ...
    <div class="is-flex-direction-column stake-range-column">
        <div class="miniBar" :style="{ width: progressSize + 'px' }">
            <div
                ...
                :class="stakeRewardedColorClass"
                :style="{ left: 0, width: stakeRewardedProgress + '%' }"
            ></div>
            <div
                class="miniBarProgress has-background-info"
                :style="{
                    left: stakeRewardedProgress + '%',
                    width: stakeNotRewardedProgress + '%',
                }"
            ></div>
            <div
                class="miniBarProgress"
        ...
        <div class="is-flex">
            <img
                alt="Minimum staking mark"
                src="@/assets/min-mark.png"
                class="image"
                style="max-height: 8px"
                :style="{ 'margin-left': minStakePix }"
            />
            <img
                alt="Maximum staking mark"
                src="@/assets/max-mark.png"
                class="image"
                style="max-height: 8px"
...
export default defineComponent({
    ...
    setup(props) {
        const minStake = computed(() => props.node?.min_stake ?? null);
        const maxStake = computed(() => props.node?.max_stake ?? null);

        const unclampedStake = computed(
            () =>
                (props.node?.stake_rewarded ?? 0) +
                (props.node?.stake_not_rewarded ?? 0),
        );
        ...
        const stakeProgress = computed(
            () => stakeRewardedProgress.value + stakeNotRewardedProgress.value,
        );

        const stakeRewardedColorClass = computed(() =>
            unclampedStake.value &&
            minStake.value &&
            unclampedStake.value < minStake.value
                ? "has-background-success"
                : props.node?.stake_rewarded &&
                  maxStake.value &&
                  props.node.stake_rewarded <= maxStake.value
                ? "has-background-success"
                : "has-background-success",
        );

        const minStakePercent = computed(() =>
            minStake.value && progressScale.value
                ? (minStake.value / progressScale.value) * 100
                : 0,
        );
        const minStakePix = computed(() => {
            const pixels = Math.round(
                (minStakePercent.value / 100) * progressSize,
            );
            return pixels.toString() + "px";
        });

        const maxStakePercent = computed(() =>
            maxStake.value && progressScale.value
                ? (maxStake.value / progressScale.value) * 100
                : 0,
        );
        const maxStakePix = computed(() => {
            const pixels = Math.round(
                ((maxStakePercent.value - minStakePercent.value) / 100) *
                    progressSize -
                    20,
            );
            return pixels.toString() + "px";
