...
export class TokenInfoAnalyzer {
    ...
    public readonly ethereumAddress = computed(() =>
        this.tokenInfo.value !== null
            ? makeEthAddressForToken(this.tokenInfo.value)
            : null,
    );
    ...
    public readonly isFungible = computed(() =>
        this.tokenInfo.value != null
            ? this.tokenInfo.value.type == "FUNGIBLE_COMMON"
            : null,
    );

    public readonly isNft = computed(() =>
        this.tokenInfo.value != null
            ? this.tokenInfo.value.type == "NON_FUNGIBLE_UNIQUE"
            : null,
    );

    public readonly hasFixedFees = computed(
        () =>
            this.tokenInfo.value?.custom_fees?.fixed_fees &&
            this.tokenInfo.value.custom_fees.fixed_fees.length > 0,
    );

    public readonly hasFractionalFees = computed(
        () =>
            this.tokenInfo.value?.custom_fees?.fractional_fees &&
            this.tokenInfo.value.custom_fees.fractional_fees.length > 0,
    );

    public readonly hasRoyaltyFees = computed(
        () =>
            this.tokenInfo.value?.custom_fees?.royalty_fees &&
            this.tokenInfo.value.custom_fees.royalty_fees.length > 0,
    );

    public readonly hasCustomFees = computed(
        () =>
            this.hasFixedFees.value ||
            this.hasFractionalFees.value ||
            this.hasRoyaltyFees.value,
    );
    ...
    public readonly customFees = computed(
        () => this.tokenInfo.value?.custom_fees,
    );
