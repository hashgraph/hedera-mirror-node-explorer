...
<template>
    ...
    <NftTransferGraph
        ...
        v-bind:class="{
            'mb-4':
                !compact && (displayTokenTransfers || displayRewardTransfers),
        }"
        v-bind:transaction="transaction"
    ...
    setup(props) {
        const netAmount = computed(() => {
            return props.transaction
                ? computeNetAmount(
                      props.transaction.transfers,
                      props.transaction.charged_tx_fee,
                  )
                : 0;
        });

        const displayRewardTransfers = computed(
            () =>
                props.transaction?.staking_reward_transfers &&
                props.transaction?.staking_reward_transfers.length >= 1,
        );
        const displayNftTransfers = computed(
            () =>
                props.transaction?.nft_transfers &&
                props.transaction?.nft_transfers.length >= 1,
        );
        const displayTokenTransfers = computed(
            () =>
                props.transaction?.token_transfers &&
                props.transaction?.token_transfers.length >= 1,
        );
