...
export class TokenTransferLayout {
    ...
    public constructor(
        ...
        const nonNullSourceAccount = TokenTransferLayout.countNonNullAcounts(
            this.sources,
        );
    ...
    ): Array<TokenTransferLayout> {
        const result = new Array<TokenTransferLayout>();

        const transferSplit = TokenTransferLayout.splitByTokenID(
    ...
    private static splitByTokenID(
        transfers: Array<TokenTransfer>,
    ): Map<string | null, Array<TokenTransfer>> {
    ...
    private static countNonNullAcounts(
        transfers: Array<TokenTransfer>,
    ): number {
        let result = 0;
        for (const t of transfers) {
