...
export class HbarTransferLayout {
    ...
    public constructor(
        ...
        if (this.transaction?.transfers) {
            transfersNetOfRewards = makeNetOfRewards(
                this.transaction.transfers,
                this.transaction.staking_reward_transfers,
            );
            ...
            for (const t of positiveTransfers) {
                const isFee = isFeeTransfer(t, this.nodes);
                const operator = t.account
                    ? makeOperatorDescription(t.account, this.nodes, isFee)
                    : null;
                this.destinations.push(
                    new HbarTransferRow(t, operator ?? "Transfer", !isFee),
                );
            }
        }

        // Makes sure net amount is distributed across payload transfers
        let remaining = this.transaction
            ? computeNetAmount(
                  transfersNetOfRewards,
                  this.transaction.charged_tx_fee,
              )
            : 0;
        ...
        if (remaining > 0) {
            for (const r of this.destinations.slice()) {
                if (!r.payload) {
                    ...
                    const payloadRow = new HbarTransferRow(
                        payloadTransfer,
                        makeOperatorDescription(
                            replacedTransfer.transfer.account ?? "",
                            this.nodes,
                            false,
                        ),
                        true,
                    ...
                    if (feeAmount > 0) {
                        ...
                        const feeRow = new HbarTransferRow(
                            feeTransfer,
                            replacedTransfer.description,
                            false,
                        );
        ...
        if (!full) {
            ...
            const sRemovalCount = HbarTransferLayout.removeFeeRows(
                this.sources,
            );
            const dRemovalCount = HbarTransferLayout.removeFeeRows(
                this.destinations,
            );
            if (
                (sRemovalCount >= 1 && this.sources.length == 0) ||
