        ...
        >
            ...
            <template v-for="i in nftTransferLayout.length" v-bind:key="i">
                ...
                <div>
                    <AccountLink
                        v-bind:account-id="
                            nftTransferLayout[i - 1].sender_account_id
                        "
                ...
                <div>
                    <TokenLink
                        v-bind:token-id="
                            nftTransferLayout[i - 1].token_id ?? undefined
                        "
                    ...
                    <div class="h-is-text-size-3" style="max-width: 200px">
                        <template v-if="!compact">
                            <span
                                v-for="sn in nftTransferLayout[i - 1]
                                    .serial_numbers"
                                :key="sn"
                            >
                                #{{ sn }}
                ...
                <div>
                    <AccountLink
                        v-bind:account-id="
                            nftTransferLayout[i - 1].receiver_account_id
                        "
                        v-bind:no-anchor="compact"
                        null-label="BURN"
...
export default defineComponent({
    ...
    setup(props) {
        const nftTransferLayout = ref(
            NFTTransferLayout.make(props.transaction),
        );

        watch(
            () => props.transaction,
            () => {
                nftTransferLayout.value = NFTTransferLayout.make(
                    props.transaction,
                );
            },
        );
