...
<template>
    <div v-if="hbarTransferLayout.rowCount >= 1">
        ...
        <div
            ...
            v-bind:class="{ 'graph-container-8': dollarVisible }"
        >
            ...
            <template v-for="i in hbarTransferLayout.rowCount" v-bind:key="i">
                ...
                <div>
                    <AccountLink
                        ...
                        v-bind:account-id="
                            hbarTransferLayout.sources[i - 1].transfer.account
                        "
                        null-label="MINT"
                ...
                <div class="justify-end">
                    <HbarAmount
                        v-if="i <= hbarTransferLayout.sources.length"
                        v-bind:amount="
                            hbarTransferLayout.sources[i - 1].transfer.amount
                        "
                        v-bind:colored="true"
                    />
                </div>
                ...
                <template v-if="dollarVisible">
                    ...
                    <div class="justify-end">
                        <HbarExtra
                            v-if="i <= hbarTransferLayout.sources.length"
                            v-bind:tbarAmount="
                                hbarTransferLayout.sources[i - 1].transfer
                                    .amount
                            "
                ...
                <div style="position: relative">
                    <ArrowSegment
                        ...
                        v-bind:dest-count="
                            hbarTransferLayout.destinations.length
                        "
                ...
                <div
                    v-bind:class="{
                        'h-has-low-contrast': hasLowContrast(i - 1),
                    }"
                >
                    <AccountLink
                        v-if="i <= hbarTransferLayout.destinations.length"
                        v-bind:account-id="
                            hbarTransferLayout.destinations[i - 1].transfer
                                .account
                        "
                        null-label="BURN"
                ...
                <template v-if="dollarVisible">
                    ...
                    >
                        ...
                        >
                            {{
                                hbarTransferLayout.destinations[i - 1]
                                    .description
                            }}
                        </span>
                    </div>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const hbarTransferLayout = computed(
            () =>
                new HbarTransferLayout(
                    props.transaction,
                    networkAnalyzer.nodes.value,
                ),
        );
        ...
        function hasLowContrast(i: number): boolean {
            ...
            const destination =
                i < destinations.length ? destinations[i] : null;
