...
export function makeTokenSymbol(
    ...
    const candidate1 =
        symbol && symbolUsable && symbol.length <= maxLength ? symbol : null;
    const candidate2 = name && name.length <= maxLength ? name : null;
    const candidate3 =
        symbol && symbolUsable ? symbol.slice(0, maxLength) : null;
...
export function makeNodeOwnerDescription(
    node: NetworkNode,
    short = false,
): string {
    const description = makeNodeDescription(node);
...
export function makeOperatorDescription(
    accountId: string,
    nodes: NetworkNode[],
    isFee = false,
): string | null {
    ...
    if (accountId === "0.0.98") {
        ...
        result =
            node !== null
                ? isFee
                    ? `Node fee (${makeNodeOwnerDescription(node, true)})`
                    : makeNodeDescription(node)
                : null;
    }
    return result;
}
...
export function decodeSolidityErrorMessage(
    message: string | null,
): string | null {
    let result: string | null;
    ...
    try {
        if (message === null) {
            ...
            result =
                "Panic(0x" + parseInt(code.toString()).toString(16) + ")" ??
                null;
        } else {
...
export function makeStakePercentage(
    node: NetworkNode,
    stakeTotal: number,
): string {
...
export function lookupNodeByAccountId(
    accountId: string,
    nodes: NetworkNode[],
): NetworkNode | null {
