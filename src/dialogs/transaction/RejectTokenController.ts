...
export class RejectTokenController extends TransactionController {
    ...
    private readonly transactionIds = ref<string[]>([])
    private readonly transactionErrors = ref<unknown[]>([])
    ...
    protected async executeTransaction(): Promise<string|null> {
        ...
        for (const t of this.makeTransactions()) {
            try {
                const tid = await walletManager.rejectTokens(t)
                if (tid !== null) {
                    this.transactionIds.value.push(tid)
                }
            } catch(error) {
        ...
        let result: string|null
        if (this.transactionIds.value.length >= 1) {
            // At least one of the transaction did complete and return its id
            result = this.transactionIds.value[0]
        } else if (this.transactionErrors.value.length == this.rejectCandidates.value.length) {
            // All transactions have failed :(
            throw this.transactionErrors.value[0]
        } else {
            // Some transactions completed but none returned its id
            result = null
        }
    ...
    private makeTransactions(): TokenRejectTransaction[] {
        const result: TokenRejectTransaction[] = []
