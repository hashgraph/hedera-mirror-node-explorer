...
export class ComplexKeyLine {
    ...
    public static flattenComplexKey(
        key: hashgraph.proto.Key,
    ): ComplexKeyLine[] {
    ...
    public contractId(): string | null {
        return this.key.contractID
            ? ComplexKeyLine.makeContractId(this.key.contractID)
            : null;
    }

    public delegatableContractId(): string | null {
        return this.key.delegatableContractId
            ? ComplexKeyLine.makeContractId(this.key.delegatableContractId)
            : null;
    }
    ...
    private static flattenComplexKeyRec(
        key: hashgraph.proto.Key,
        level: number,
        result: ComplexKeyLine[],
    ): void {
        ...
        if (key.keyList) {
            ...
            if (
                key.thresholdKey.keys?.keys &&
                key.thresholdKey.keys?.keys.length == 1
            ) {
    ...
    private static makeContractId(
        iContractID: hashgraph.proto.IContractID,
    ): string {
