...
export class HCSAssetCache extends EntityCache<string, HCSAsset | null> {
    ...
    public readonly drained = computed(() => this.drainedRef.value)
    ...
    protected async load(topicId: string): Promise<HCSAsset | null> {
        ...
        const url: string | null = `api/v1/topics/${topicId}/messages?limit=100&order=asc`
        const response: AxiosResponse<TopicMessagesResponse> = await axios.get<TopicMessagesResponse>(url)
        this.drainedRef.value = response.data.links.next === null

        if (response.data.messages.length >= 1) {
            const asset = await HCSAsset.reassemble(response.data.messages)
            if (asset != null) {
    ...
    // Private
    //
    private drainedRef = ref<boolean | null>(null)
