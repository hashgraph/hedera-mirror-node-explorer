...
export class WalletConnectAgent {
    ...
    private static makeCaChainForEIP155(network: string): string|null {
        let result: string|null
        const chainId = networkRegistry.lookup(network)?.sourcifySetup?.chainID ?? null
        if (chainId !== null) {
    ...
    public async makeClient(accountId: string): Promise<WalletClient|null> {
        ...
        if (caAccountId !== null) {
            ...
            if (caChainId.isHedera()) {
                result = new WalletClient_Hedera(accountId, routeManager.currentNetwork.value, provider)
