...
import {WalletClient_Hedera} from "@/utils/wallet/client/WalletClient_Hedera";
...
export class WalletManagerV4 {
    ...
    private readonly projectId = import.meta.env.VITE_WALLET_CONNECT_PROJECT_ID ?? null
    ...
    public readonly isHederaWallet
        = computed(() => this.client.value instanceof WalletClient_Hedera)
    ...
    public async rejectTokens(transaction: TokenRejectTransaction): Promise<string> {
        if (this.client.value instanceof WalletClient_Hedera) {
    ...
    private async initWalletConnect(): Promise<void> {
        if (this.projectId !== null) {
            try {
                this.walletConnectAgent = await WalletConnectAgent.makeInstance(this.projectId)
            } catch(error) {
