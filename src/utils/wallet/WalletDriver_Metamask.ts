...
export class WalletDriver_Metamask extends WalletDriver {
    ...
    public async changeAccount(network: string): Promise<string[]> {
        ...
        const accounts = await provider?.request({
            // request new permissions to prompt up new account list menu
            ...
            // Collects account ids
            const result = await this.fetchAccountIds(provider)
            if (result.length == 0) {
                // Did the user cancel connection from the wallet ?
                throw this.changeAccountFailure("No account found - make sure to connect to a valid account")
            }
            return result;
        })
        ...
        if (!accounts) {
            // Did the user cancel connection from the wallet ?
            throw this.changeAccountFailure("No account found")
        }
