...
export class PathParam {
    ...
    public static parseBlockLoc(
        s: string,
    ): number | TransactionHash | EthereumHash | null {
    ...
    public static parseAccountLoc(
        l: string,
    ): EntityID | EthereumAddress | AccountAlias | null {
        return (
            EntityID.parse(l) ??
            EthereumAddress.parse(l) ??
            AccountAlias.parse(l)
        );
    }

    public static parseAccountIdOrAliasOrEvmAddress(
        s: string | undefined,
    ): string | null {
    ...
    public static parseTransactionIdOrHash(
        s: string | undefined,
    ): string | null {
        let result: string | null;
    ...
    public static parseTransactionLoc(
        s: string,
    ): Timestamp | TransactionHash | EthereumHash | null {
        return (
            Timestamp.parse(s) ??
            TransactionHash.parse(s) ??
            EthereumHash.parse(s)
        );
    }
