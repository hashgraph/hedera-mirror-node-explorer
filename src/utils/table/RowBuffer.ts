...
export class RowBuffer<R, K> {
    ...
    public constructor(
        controller: TableController<R, K>,
    ...
    public readonly tailKey: ComputedRef<K | null> = computed(() => {
        ...
        const tailRow =
            bufferLength >= 1 ? this.rows.value[bufferLength - 1] : null;
    ...
    public readonly maxStartIndex: ComputedRef<number> = computed(() => {
        ...
        const pageCount =
            Math.floor(
                (this.shadowRowCount.value + this.rows.value.length) / pageSize,
            ) + 1;
    ...
    public computeFirstVisibleKey(): K | null {
        const bufferLength = this.rows.value.length;
        const firstRow =
            this.startIndex.value < bufferLength
                ? this.rows.value[this.startIndex.value]
                : null;
    ...
    public async refresh(): Promise<void> {
        ...
        if (this.headKey.value !== null) {
            ...
            if (this.refreshCounter == capturedRefreshCounter) {
                if (newRows !== null) {
                    this.rows.value = this.concatOrReplace(
                        newRows,
                        this.rows.value,
                    );
            ...
            if (this.refreshCounter == capturedRefreshCounter) {
                if (
                    newRows !== null &&
                    this.refreshCounter == capturedRefreshCounter
                ) {
            ...
            if (key !== null) {
                ...
                if (this.moveToPageCounter == captureMoveToPageCounter) {
                    if (newRows !== null) {
                        ...
                        this.drained.value = newRows.length < pageSize;
                        this.startIndex.value = Math.min(
                            nextStartIndex - shadowRowCount,
                            this.maxStartIndex.value,
                        );
    ...
    private headLoad(
        ...
        if (rowCount >= 1) {
            ...
            const limitedCount = Math.min(
                this.tableController.maxLimit,
                rowCount,
            );
            const remainingCount = rowCount - limitedCount;
            result = this.tableController
                .load(key, KeyOperator.gt, SortOrder.ASC, limitedCount)
                .then(cb);
        } else {
            result = Promise.resolve(current);
        }
    ...
    private tailLoad(
        ...
        if (rowCount >= 1) {
            ...
            result = this.tableController
                .load(key, operator, SortOrder.DESC, limitedCount)
                .then(cb);
        } else {
            result = Promise.resolve(current);
        }
    ...
    private lastLoad(
        ...
        if (rowCount >= 1) {
            ...
            const remainingCount = rowCount - limitedCount;
            result = this.tableController
                .load(key, KeyOperator.gte, SortOrder.DESC, limitedCount)
                .then(cb);
        } else {
            result = Promise.resolve(current);
        }
    ...
    private concatOrReplace(newRows: R[], currentRows: R[]): R[] {
        ...
        const newTailRow =
            newRows.length >= 1 ? newRows[newRows.length - 1] : null;
        const newTailKey =
            newTailRow !== null
                ? this.tableController.keyFor(newTailRow)
                : null;
        const currentHeadRow = currentRows.length >= 1 ? currentRows[0] : null;
        const currentHeadKey = currentHeadRow
            ? this.tableController.keyFor(currentHeadRow)
            : null;
