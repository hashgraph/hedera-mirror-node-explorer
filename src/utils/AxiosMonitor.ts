...
export class AxiosMonitor {
    ...
    public setTargetAxios(newValue: Axios | null): void {
        if (this.targetAxios !== null) {
            if (this.requestInterceptor !== null) {
                this.targetAxios.interceptors.request.eject(
                    this.requestInterceptor,
                );
                this.requestInterceptor = null;
            }
            if (this.responseInterceptor !== null) {
                this.targetAxios.interceptors.response.eject(
        ...
        if (this.targetAxios !== null) {
            ...
            this.responseInterceptor =
                this.targetAxios.interceptors.response.use(
    ...
    public clearErrorResponses(): void {
        if (this.errorResponses.size >= 1) {
            console.log(
                "Clearing " + this.errorResponses.size + " error responses",
            );
    ...
    private requestFullfilled(response: AxiosResponse): Promise<AxiosResponse> {
        ...
        if (this.targetAxios !== null) {
            ...
            this.errorResponses.delete(
                this.targetAxios.getUri(response.config),
            );
            this.stateDidChange();
    ...
    private requestRejected(reason: unknown): unknown {
        ...
        if (this.targetAxios !== null) {
            ...
            if (axios.isAxiosError(reason) && reason.request?.status != 404) {
                this.errorResponses.set(
                    this.targetAxios.getUri(reason.config),
                    reason,
                );
            }
            this.stateDidChange();
    ...
    private stateDidChange(): void {
        ...
        this.initialLoading.value =
            this.activeRequestCount >= 1 && this.idlePeriodCount == 0;
