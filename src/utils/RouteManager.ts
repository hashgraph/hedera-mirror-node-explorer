...
export class RouteManager {
    ...
    public readonly currentNetworkEntry = computed(() => {
        ...
        const networkEntry =
            networkName != null ? networkRegistry.lookup(networkName) : null;

        return networkEntry != null
            ? networkEntry
    ...
    public updateSelectedNetworkSilently(): void {
        ...
        this.selectedNetworkWatchHandle = watch(
    ...
    public readonly previousRoute = computed(
        () => this.router.currentRoute.value?.query.from as string,
    );

    public readonly isDashboardRoute = computed(() =>
        this.testDashboardRoute(),
    );
    public readonly isTransactionRoute = computed(() =>
        this.testTransactionRoute(),
    );
    ...
    public routeToTransaction(
        ...
        if (newTab) {
            const routeData = this.router.resolve(
                this.makeRouteToTransaction(t.consensus_timestamp),
            );
    ...
    public routeToTransactionByTs(
        consensusTimestamp: string | undefined,
        newTab = false,
    ): Promise<NavigationFailure | void | undefined> {
        let result: Promise<NavigationFailure | void | undefined>;
        if (newTab) {
            const routeData = this.router.resolve(
                this.makeRouteToTransaction(consensusTimestamp),
            );
            window.open(routeData.href, "_blank");
            result = Promise.resolve();
        } else {
    ...
    public makeRouteToTransactionObj(
        transaction: Transaction,
    ): RouteLocationRaw {
        return this.makeRouteToTransaction(transaction.consensus_timestamp);
    }

    public makeRouteToTransaction(
        transactionLoc: string | undefined,
    ): RouteLocationRaw {
        return {
            name: "TransactionDetails",
            params: {
                transactionLoc: transactionLoc,
    ...
    public routeToTransactionsById(
        transactionId: string,
    ): Promise<NavigationFailure | void | undefined> {
        return this.router.push(
            this.makeRouteToTransactionsById(transactionId),
        );
    }

    public makeRouteToTransactionsById(
        transactionId: string,
    ): RouteLocationRaw {
        return {
    ...
    public routeToAccount(
        ...
        if (newTab) {
            const routeData = this.router.resolve(
                this.makeRouteToAccount(accountId),
            );
            window.open(routeData.href, "_blank");
            result = Promise.resolve();
        } else {
    ...
    public makeRouteToAccountsWithKey(pubKey: string): RouteLocationRaw {
        return {
            ...
            params: { pubKey: pubKey, network: this.currentNetwork.value },
        };
    }

    public routeToAccountsWithKey(
        pubKey: string,
    ): Promise<NavigationFailure | void | undefined> {
    ...
    public makeRouteToToken(tokenId: string): RouteLocationRaw {
        return {
            name: "TokenDetails",
            params: { tokenId: tokenId, network: this.currentNetwork.value },
        };
    }
    ...
    public routeToContract(
        ...
        if (newTab) {
            const routeData = this.router.resolve(
                this.makeRouteToContract(contractId),
            );
            window.open(routeData.href, "_blank");
            result = Promise.resolve();
        } else {
    ...
    public makeRouteToTopic(topicId: string): RouteLocationRaw {
        return {
            name: "TopicDetails",
            params: { topicId: topicId, network: this.currentNetwork.value },
        };
    }
    ...
    public makeRouteToBlock(blockHon: string | number): RouteLocationRaw {
        return {
            name: "BlockDetails",
            params: { blockHon: blockHon, network: this.currentNetwork.value },
        };
    }

    public routeToBlock(
        blockHon: string | number,
        newTab = false,
    ): Promise<NavigationFailure | void | undefined> {
        let result: Promise<NavigationFailure | void | undefined>;
        if (newTab) {
            const routeData = this.router.resolve(
                this.makeRouteToBlock(blockHon),
            );
            window.open(routeData.href, "_blank");
            result = Promise.resolve();
        } else {
    ...
    public makeRouteToNode(nodeId: number): RouteLocationRaw {
        return {
            name: "NodeDetails",
            params: { nodeId: nodeId, network: this.currentNetwork.value },
        };
    }
    ...
    public makeRouteToNoSearchResult(
        searchedId: string,
        errorCount: number,
    ): RouteLocationRaw {
        return {
    ...
    public routeToNoSearchResult(
        searchedId: string,
        errorCount: number,
    ): Promise<NavigationFailure | void | undefined> {
        return this.router.push(
            this.makeRouteToNoSearchResult(searchedId, errorCount),
        );
    }
    ...
    public routeToMainDashboard(): Promise<
        NavigationFailure | void | undefined
    > {
    ...
    private switchThemes() {
        if (
            this.currentNetworkEntry.value.name == NetworkRegistry.TEST_NETWORK
        ) {
            document.documentElement.style.setProperty(
                "--h-theme-background-color",
                "var(--h-testnet-background-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-highlight-color",
                "var(--h-testnet-highlight-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-pagination-background-color",
                "var(--h-testnet-pagination-background-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-box-shadow-color",
                "var(--h-testnet-box-shadow-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-dropdown-arrow",
                "var(--h-testnet-dropdown-arrow)",
            );
        } else if (
            this.currentNetworkEntry.value.name ==
            NetworkRegistry.PREVIEW_NETWORK
        ) {
            document.documentElement.style.setProperty(
                "--h-theme-background-color",
                "var(--h-previewnet-background-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-highlight-color",
                "var(--h-previewnet-highlight-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-pagination-background-color",
                "var(--h-previewnet-pagination-background-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-box-shadow-color",
                "var(--h-previewnet-box-shadow-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-dropdown-arrow",
                "var(--h-previewnet-dropdown-arrow)",
            );
        } else {
            document.documentElement.style.setProperty(
                "--h-theme-background-color",
                "var(--h-mainnet-background-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-highlight-color",
                "var(--h-mainnet-highlight-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-pagination-background-color",
                "var(--h-mainnet-pagination-background-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-box-shadow-color",
                "var(--h-mainnet-box-shadow-color)",
            );
            document.documentElement.style.setProperty(
                "--h-theme-dropdown-arrow",
                "var(--h-mainnet-dropdown-arrow)",
            );
        }
    }
...
export function fetchStringQueryParam(
    paramName: string,
...
export function fetchNumberQueryParam(
    paramName: string,
    route: RouteLocationNormalizedLoaded,
): number | null {
