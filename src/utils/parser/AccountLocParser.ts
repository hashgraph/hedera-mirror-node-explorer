...
export class AccountLocParser {
    ...
    public readonly accountInfo: Ref<AccountBalanceTransactions | null> =
        ref(null);
        ...
        if (this.watchHandle.value !== null) {
            ...
            if (this.accountInfo.value !== null) {
                ...
                if (this.accountInfo.value.account) {
                    AccountByIdCache.instance.forget(
                        this.accountInfo.value.account,
                    );
                }
                if (this.accountInfo.value.alias) {
                    AccountByAliasCache.instance.forget(
                        this.accountInfo.value.alias,
                    );
                }
                const address = makeEthAddressForAccount(
    ...
    public readonly accountId: ComputedRef<string | null> = computed(
        () => this.accountInfo.value?.account ?? null,
    );
    ...
    public readonly balance: Ref<number | null> = computed(
        () => this.accountInfo.value?.balance?.balance ?? null,
    );

    public readonly createdTimestamp: Ref<string | null> = computed(
        () => this.accountInfo.value?.created_timestamp ?? null,
    );

    public readonly key: Ref<Key | null> = computed(
        () => this.accountInfo.value?.key ?? null,
    );

    public readonly tokens: Ref<TokenBalance[] | null> = computed(
        () => this.accountInfo.value?.balance?.tokens ?? null,
    );

    public readonly stakedNodeId: Ref<number | null> = computed(
        () => this.accountInfo.value?.staked_node_id ?? null,
    );

    public readonly stakedAccountId: Ref<string | null> = computed(
    ...
    public readonly stakePeriodStart: Ref<string | null> = computed(() => {
        ...
        if (this.accountInfo.value?.stake_period_start) {
            result = dateFormat.format(
                Number.parseFloat(this.accountInfo.value.stake_period_start) *
                    1000,
            );
        } else {
            result = null;
    ...
    public readonly pendingReward: Ref<number | null> = computed(
        () => this.accountInfo.value?.pending_reward ?? null,
    );

    public readonly accountDescription: Ref<string | null> = computed(
        () => this.nodeAnalyzer.nodeDescription.value,
    );

    public readonly nodeId: Ref<number | null> = computed(
        () => this.nodeAnalyzer.node.value?.node_id ?? null,
    );

    public readonly ethereumAddress = computed(() => {
        return this.accountInfo.value !== null
            ? makeEthAddressForAccount(this.accountInfo.value)
            : null;
    });
            ...
            if (o !== null) {
                if (a !== null) {
                    ...
                    if (o instanceof EntityID) {
                        ...
                        result =
                            "Own this account? " +
                            "Activate it by transferring any amount of ℏ or tokens to " +
                            o.toCompactString() +
                            ".";
                    } else {
                        // o instanceof AccountAlias
                        result = "Account with alias " + o + " was not found";
                    }
                } else {
                    // this.loadCounter.value === 0 => not loaded yet
                    result = null;
                }
            } else {
        ...
        if (l !== null) {
            try {
                if (l instanceof EntityID) {
                    this.accountInfo.value =
                        await AccountByIdCache.instance.lookup(l.toString());
                } else if (l instanceof EthereumAddress) {
                    this.accountInfo.value =
                        await AccountByAddressCache.instance.lookup(
                            l.toString(),
                        );
                } else {
                    // l instanceof AccountAlias
                    this.accountInfo.value =
                        await AccountByAliasCache.instance.lookup(l.toString());
                }
            } catch (error) {
                this.accountInfo.value = null;
            } finally {
