...
export class TransactionLocParser {
    ...
    public mount(): void {
        this.watchHandle.value = watch(
            this.transactionLocObj,
            this.transactionLocObjDidChange,
            { immediate: true },
        );
    }
            ...
            if (o !== null) {
                if (t !== null) {
                    ...
                    if (o instanceof Timestamp) {
                        ...
                        result =
                            "Transaction with hash " + o + " was not found";
                    } else if (o instanceof EthereumHash) {
                        result =
                            "Transaction with ethereum hash " +
                            o +
                            " was not found";
                    } else {
                        result = null;
                    ...
                    // this.loadCounter.value === 0 => no loading yet
                    result = null;
                }
            } else {
                result =
                    "Invalid transaction id, timestamp or hash: " +
                    this.transactionLoc.value;
            }
        } else {
            result = null;
        }
            ...
            try {
                if (o instanceof Timestamp) {
                    this.transactionRef.value =
                        await TransactionByTsCache.instance.lookup(
                            o.toString(),
                        );
                } else if (o instanceof TransactionHash) {
                    this.transactionRef.value =
                        await TransactionByHashCache.instance.lookup(
                            o.toString(),
                        );
                } else if (o instanceof TransactionID) {
                    this.transactionRef.value =
                        await TransactionByIdCache.instance.lookup(
                            o.toString(false),
                        );
                } else {
                    // => o instanceof EthereumHash
                    this.transactionRef.value =
                        await this.lookupTransactionByResult(o.toString());
                }
            } catch {
                this.transactionRef.value = null;
            } finally {
    ...
    private readonly transactionLocObj = computed(() => {
        ...
        if (l !== null) {
            result =
                Timestamp.parse(l) ??
                TransactionHash.parse(l) ??
                TransactionID.parse(l) ??
                EthereumHash.parse(l);
        } else {
            result = null;
        }
        return result;
    ...
    private async lookupTransactionByResult(
        ethereumHash: string,
    ): Promise<Transaction | null> {
        let result: Transaction | null;
        const contractResult =
            await ContractResultByHashCache.instance.lookup(ethereumHash);
        const consensusTimestamp = contractResult?.timestamp ?? null;
        if (consensusTimestamp !== null) {
            result =
                await TransactionByTsCache.instance.lookup(consensusTimestamp);
        } else {
            result = null;
        }
