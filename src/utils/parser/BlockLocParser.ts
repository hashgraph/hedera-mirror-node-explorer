...
export class BlockLocParser {
    ...
    public readonly blockNumber: ComputedRef<number | null> = computed(
        () => this.block.value?.number ?? null,
    );
    public readonly blockCount: ComputedRef<number | null> = computed(
        () => this.block.value?.count ?? null,
    );
    public readonly toTimestamp: ComputedRef<string | null> = computed(
        () => this.block.value?.timestamp?.to ?? null,
    );
        ...
        if (l !== null) {
            try {
                if (typeof l == "number") {
                    ...
                    // l instanceof EthereumHash
                    this.block.value = await BlockByHashCache.instance.lookup(
                        l.toString(),
                    );
                }
            } catch (error) {
