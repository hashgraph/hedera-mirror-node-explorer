...
export abstract class EntityDownloader<E, R> {
    ...
    public async run(): Promise<void> {
        ...
        try {
            ...
            this.stateRef.value = this.abortRequested
                ? DownloaderState.Aborted
    ...
    public readonly state: ComputedRef<DownloaderState> = computed(
        () => this.stateRef.value,
    );
    public readonly downloadedCount: ComputedRef<number> = computed(
        () => this.downloadedCountRef.value,
    );
    public readonly firstDownloadedEntity: ComputedRef<E | null> = computed(
        () => this.firstDownloadedEntityRef.value,
    );
    public readonly lastDownloadedEntity: ComputedRef<E | null> = computed(
        () => this.lastDownloadedEntityRef.value,
    );
    public readonly drained: ComputedRef<boolean> = computed(
        () => this.drainedRef.value,
    );
    public readonly failureReason: ComputedRef<unknown | null> = computed(
        () => this.failureReasonRef.value,
    );
    ...
    public progress: ComputedRef<number> = computed(() => {
        ...
        if (this.state.value == DownloaderState.Completed) {
            ...
            const firstTimestamp = firstEntity
                ? this.entityTimestamp(firstEntity)
                : null;
            const firstTime =
                firstTimestamp !== null
            ...
            const lastTimestamp = lastEntity
                ? this.entityTimestamp(lastEntity)
                : null;
            const lastTime =
                lastTimestamp !== null
            ...
            const progress =
                firstTime !== null && lastTime !== null
                    ? (firstTime - lastTime) / (firstTime - startTime)
                    : 0;
    ...
    public getOutputName(): string {
        ...
        if (this.startDate.value !== null) {
            result =
                this.makeOutputPrefix() +
                " " +
                this.dateFormat.format(this.startDate.value) +
                " to " +
                this.dateFormat.format(this.endDate.value ?? this.now) +
                ".csv";
        } else {
            result = "";
        }
    ...
    public csvBlob: ComputedRef<Blob | null> = computed(() => {
        ...
        if (
            this.state.value == DownloaderState.Completed &&
            this.downloadedCount.value >= 1
        ) {
    ...
    protected abstract makeCSVEncoder(
    ...
    private async download(): Promise<void> {
        ...
        let nextURL = null;
        while (
            this.entities.length < this.maxEntityCount &&
            !this.drainedRef.value &&
            !this.abortRequested
        ) {
            const newResponse = await this.loadNext(nextURL);
            const newEntities = this.filter(
                this.fetchEntities(newResponse.data),
            );
            this.entities = this.entities.concat(newEntities);
            this.downloadedCountRef.value = this.entities.length;
            this.firstDownloadedEntityRef.value =
