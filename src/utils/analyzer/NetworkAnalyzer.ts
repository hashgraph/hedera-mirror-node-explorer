...
export class NetworkAnalyzer {
    ...
    public readonly nodes = computed(
        () => this.networkLookup.entity.value ?? [],
    );

    public readonly nodeCount = computed(() => this.nodes.value?.length ?? 0);

    public readonly node0 = computed(() =>
        this.nodes.value.length >= 1 ? this.nodes.value[0] : null,
    );
    ...
    public readonly totalRewarded: ComputedRef<number> = computed(() => {
        ...
        for (const n of this.nodes.value) {
            result +=
                ((n.reward_rate_start ?? 0) * (n.stake_rewarded ?? 0)) /
                100000000;
        }
        return result;
    });
        ...
        for (const n of this.nodes.value) {
            if (isCouncilNode(n)) {
                const thisMax = Math.max(
                    n.max_stake ?? 0,
                    (n.stake_rewarded ?? 0) + (n.stake_not_rewarded ?? 0),
                );
                result += thisMax;
                nbNodes++;
            }
    ...
    public readonly durationMin = computed(
        () => this.stakingPeriod.value?.durationMin ?? null,
    );
    public readonly elapsedMin = computed(
        () => this.stakingPeriod.value?.elapsedMin ?? null,
    );
    public readonly remainingMin = computed(
        () => this.stakingPeriod.value?.remainingMin ?? null,
    );
    ...
    private readonly updateStakingPeriod = () => {
        ...
        if (node0 !== null) {
            startTimeInSec = node0.staking_period?.from
                ? Number.parseInt(node0.staking_period?.from)
                : null;
            endTimeInSec = node0.staking_period?.to
                ? Number.parseInt(node0.staking_period?.to)
                : null;
        } else {
            startTimeInSec = null;
            endTimeInSec = null;
        }
        this.stakingPeriod.value = new StakingPeriod(
            startTimeInSec,
            endTimeInSec,
        );
    };
