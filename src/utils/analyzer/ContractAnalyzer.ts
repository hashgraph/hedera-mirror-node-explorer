...
} from "@/schemas/SystemContractRegistry";
...
export class ContractAnalyzer {
    ...
    private readonly systemContractEntry: Ref<SystemContractEntry | null> =
        ref(null);
    ...
    public mount(): void {
        ...
        this.watchHandles = [
            watch(this.contractId, this.contractIdDidChange, {
                immediate: true,
            }),
            watch(this.contractResponse, this.contractResponseDidChange, {
                immediate: true,
            }),
            watch([this.systemContractEntry, this.metadata], this.updateABI, {
                immediate: true,
            }),
            watch(
                [this.metadata, this.sourceAnalyzers],
                this.updateMissingSourceCount,
    ...
    public readonly globalState = computed<GlobalState | null>(() => {
        ...
        if (this.contractId.value !== null) {
            if (this.sourcifyRecord.value !== null) {
                result = this.sourcifyRecord.value.fullMatch
                    ? GlobalState.FullMatch
                    : GlobalState.PartialMatch;
            } else if (this.metadata.value !== null) {
                result =
                    this.missingSourceCount.value >= 1
                        ? GlobalState.MissingSources
                        : GlobalState.ReadyToVerifiy;
            } else {
    ...
    public readonly metadata: ComputedRef<SolcMetadata | null> = computed(
        () => {
            ...
            if (this.sourcifyRecord.value !== null) {
                result = SourcifyCache.fetchMetadata(
                    this.sourcifyRecord.value.response,
                );
    ...
    public readonly metadataOrigin: ComputedRef<MetadataOrigin | null> =
        computed(() => {
    ...
    public readonly interface: ComputedRef<ethers.utils.Interface | null> =
        computed(() => {
    ...
    public readonly sourceContents: ComputedRef<Record<string, string>> =
        computed(() => {
    ...
    public readonly sourceAnalyzers: ComputedRef<ContractSourceAnalyzer[]> =
        computed(() => {
    ...
    private readonly contractIdDidChange = async () => {
        if (this.contractId.value !== null) {
            ...
            if (e !== null) {
                ...
                this.systemContractEntry.value = null;
                try {
                    this.contractResponse.value =
                        await ContractByIdCache.instance.lookup(
    ...
    private contractResponseDidChange = async () => {
        if (this.contractResponse.value?.contract_id) {
            try {
                this.sourcifyRecord.value = await SourcifyCache.instance.lookup(
                    this.contractResponse.value?.contract_id,
                );
            } catch {
                this.sourcifyRecord.value = null;
            }
    ...
    private updateABI = async () => {
        if (this.systemContractEntry.value !== null) {
            try {
                ...
                const asset = (await AssetCache.instance.lookup(abiURL)) as {
            ...
            this.abi.value = this.metadata.value.output.abi as
    ...
    private updateLocalStorageMetadata(): void {
        this.localStorageMetadata.value =
            this.contractId.value != null
                ? AppStorage.getMetadata(this.contractId.value)
                : null;
    }
}
