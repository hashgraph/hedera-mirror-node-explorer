...
<template>
    <section
        class="section is-top-section"
        :class="{ 'is-medium-screen': isMediumScreen }"
    >
        <TopNavBar />
    </section>

    <hr
        v-if="!onMainDashboardPage"
        class="h-has-background-color"
...
export default defineComponent({
    ...
    setup() {
        ...
        const buildRelease =
            import.meta.env.VITE_BUILD_RELEASE ?? "not available";
        provide("buildRelease", buildRelease);

        const buildShortCommitHash =
            import.meta.env.VITE_BUILD_SHORTCOMMITHASH ?? "not available";
        provide("buildShortCommitHash", buildShortCommitHash);

        const buildTime =
            import.meta.env.VITE_BUILD_TIME_UTC ?? "not available";
