...
describe("TransactionDetails.vue", () => {
    ...
    it("Should display transaction details with token transfers and hbar transfers", async () => {
        ...
        mock.onGet(matcher11).reply((config: AxiosRequestConfig) => {
            if (
                ...
                SAMPLE_TRANSACTION.consensus_timestamp
            ) {
        ...
        expect(wrapper.get("#transactionTypeValue").text()).toBe(
            "CRYPTO TRANSFER",
        );
        expect(wrapper.get("#consensusAtValue").text()).toBe(
            "5:12:31.6676 AMFeb 28, 2022, UTC",
        ); // UTC because of HMSF.forceUTC
        expect(wrapper.get("#transactionHashValue").text()).toBe(
            "a012 9612 32ed 7d28 4283 6e95 f7e9 c435 6fdf e2de 0819 9091 701a 969c 1d1f d936 71d3 078e e83b 28fb 460a 88b4 cbd8 ecd2Copy",
        );
        // expect(wrapper.get("#netAmountValue").text()).toBe("0.00000000$0.0000")
        expect(wrapper.get("#chargedFeeValue").text()).toBe(
            "0.00470065$0.00116",
        );
        ...
        expect(wrapper.get("#operatorAccountValue").text()).toBe(
            "0.0.29624024",
        );
        expect(wrapper.get("#nodeAccountValue").text()).toBe(
            "0.0.5Hosted by Hedera | Central, USA",
        );
    ...
    it("Should display the contract result and logs (using consensus timestamp)", async () => {
        ...
        const transactionId =
            SAMPLE_CONTRACTCALL_TRANSACTIONS.transactions[0].transaction_id;
        const contractId =
            SAMPLE_CONTRACTCALL_TRANSACTIONS.transactions[0].entity_id;
        const timestamp =
            SAMPLE_CONTRACTCALL_TRANSACTIONS.transactions[0]
                .consensus_timestamp;
        ...
        const matcher4 =
            "/api/v1/contracts/" +
            SAMPLE_CONTRACT_RESULT_DETAILS.contract_id +
            "/results/" +
            timestamp;
        mock.onGet(matcher4).reply(200, SAMPLE_CONTRACT_RESULT_DETAILS);

        const matcher5 =
            "/api/v1/contracts/results/" + transactionId + "/actions";
        ...
        expect(wrapper.text()).toMatch(
            RegExp(
                "^Transaction " + normalizeTransactionId(transactionId, true),
            ),
        );
        expect(wrapper.get("#transactionTypeValue").text()).toBe(
            "CONTRACT CALL",
        );
        expect(wrapper.get("#entityId").text()).toBe(
        ...
        expect(wrapper.get("#maxFeeName").text()).toBe(
            "Max FeeMax Fee limit does not include the hbar cost of gas consumed by transactions executed on the EVM.",
        );
        expect(wrapper.get("#maxFeeValue").text()).toBe("0.20000000$0.04921");

        expect(wrapper.findComponent(ContractResult).exists()).toBe(true);
        expect(wrapper.findComponent(ContractResult).text()).toMatch(
            RegExp("^Contract Result"),
        );
        expect(wrapper.get("#resultValue").text()).toBe("SUCCESS");
        expect(wrapper.get("#evm-hashValue").text()).toBe(
            "c43d b9ea cf72 c916 29ac 0308 8535 dd9a e410 59a2 c1ee fce3 a528 e04e 7e90 8d2dCopy",
        );
    ...
    it("Should display the contract result and logs (using transaction hash)", async () => {
        ...
        const transactionHash = byteToHex(
            base64DecToArr(transactionHashBase64),
        );
    ...
    it("Should update when consensus timestamp changes", async () => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp(
                "^Transaction " +
                    normalizeTransactionId(transaction.transaction_id, true),
            ),
        );
        expect(wrapper.get("#transactionTypeValue").text()).toBe(
            "CONTRACT CALL",
        ...
        expect(wrapper.get("#memoValue").text()).toBe(
            "Mirror Node acceptance test: 2022-03-07T15:09:26.066680977Z Execute contract",
        );
    ...
    it("Should display a notification banner for failed transaction", async () => {
        ...
        mock.onGet(matcher1).reply((config: AxiosRequestConfig) => {
            if (
                config.params.timestamp ==
                SAMPLE_FAILED_TRANSACTION.consensus_timestamp
            ) {
        ...
        expect(wrapper.text()).toMatch(
            RegExp(
                "^Transaction " +
                    normalizeTransactionId(
                        SAMPLE_FAILED_TRANSACTION.transaction_id,
                        true,
                    ),
            ),
        );
    ...
    it("Should detect invalid transaction timestamp", async () => {
        ...
        expect(wrapper.get("#notificationBanner").text()).toBe(
            "Transaction with timestamp " + invalidTimestamp + " was not found",
        );
    ...
    it("Should display the name of the system contract called", async () => {
        ...
        const transaction =
            SAMPLE_SYSTEM_CONTRACT_CALL_TRANSACTIONS.transactions[0];
        ...
        expect(wrapper.get("#entityId").text()).toBe(
            "Contract IDHedera Token Service System Contract",
        );
    ...
    it("Should display a link to the scheduled transaction", async () => {
        ...
        const SCHEDULING =
            SAMPLE_SCHEDULING_SCHEDULED_TRANSACTIONS.transactions[0];
        const SCHEDULED =
            SAMPLE_SCHEDULING_SCHEDULED_TRANSACTIONS.transactions[1];
        const TOKEN_ID = SCHEDULED.token_transfers
            ? SCHEDULED.token_transfers[0].token_id
            : "0.0.1304757";
        const matcher1 = "/api/v1/transactions";
        mock.onGet(matcher1).reply((config: AxiosRequestConfig) => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp(
                "^Transaction " +
                    normalizeTransactionId(SCHEDULING.transaction_id, true),
            ),
        );
    ...
    it("Should display a link to the scheduling transaction", async () => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp(
                "^Transaction " +
                    normalizeTransactionId(SCHEDULED.transaction_id, true),
            ),
        );
    ...
    it("Should display a link to the parent transaction", async () => {
        ...
        const TOKEN_ID = CHILD.nft_transfers
            ? CHILD.nft_transfers[0].token_id
            : "0.0.48193741";
        const matcher1 = "/api/v1/transactions";
        mock.onGet(matcher1).reply((config: AxiosRequestConfig) => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp(
                "^Transaction " +
                    normalizeTransactionId(CHILD.transaction_id, true),
            ),
        );
    ...
    it("Should display link to the child transactions", async () => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp(
                "^Transaction " +
                    normalizeTransactionId(PARENT.transaction_id, true),
            ),
        );
    ...
    it("Should NOT display a link to the parent transaction", async () => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp(
                "^Transaction " +
                    normalizeTransactionId(NONCE_1.transaction_id, true),
            ),
        );
    ...
    it("Should display transaction details with account/token association", async () => {
        ...
        const matcher3 =
            "/api/v1/accounts/" + transaction.entity_id + "/tokens?limit=100";
        ...
        expect(wrapper.get("#consensusAtValue").text()).toBe(
            "6:51:52.1505 PMDec 21, 2022, UTC",
        ); // UTC because of HMSF.forceUTC
        expect(wrapper.get("#transactionHashValue").text()).toBe(
            "4786 0799 99df 169a 3834 9249 d3c9 a548 9a83 f1c7 c51b 6b1e deb8 1347 a496 d931 83e2 4a43 ad03 372e bc50 1528 a603 2debCopy",
        );

        expect(wrapper.get("#associatedTokenIdValue").text()).toBe(
            "0.0.34332104HSuite0.0.49292859TokenA7",
        );
        ...
        expect(wrapper.findComponent(HbarTransferGraphF).text()).toBe(
            "Hbar TransfersAccountHbar AmountAccountHbar Amount0.0.642949-1.15905210-$0.28517\n\n" +
    ...
    it("Should display CONTRACT CALL details with link to (proxied) token as entity ID", async () => {
        ...
        const transactionId =
            SAMPLE_TOKEN_CALL_TRANSACTIONS.transactions[0].transaction_id;
        const entityId =
            SAMPLE_TOKEN_CALL_TRANSACTIONS.transactions[0].entity_id;
        const timestamp =
            SAMPLE_TOKEN_CALL_TRANSACTIONS.transactions[0].consensus_timestamp;
    ...
    it("Should display ETHEREUM TX details with link to account as entity ID", async () => {
        ...
        const transactionId =
            SAMPLE_ETHEREUM_TRANSACTIONS_ON_ACCOUNT.transactions[0]
                .transaction_id;
        const entityId =
            SAMPLE_ETHEREUM_TRANSACTIONS_ON_ACCOUNT.transactions[0].entity_id;
        const timestamp =
            SAMPLE_ETHEREUM_TRANSACTIONS_ON_ACCOUNT.transactions[0]
                .consensus_timestamp;

        const matcher1 = "/api/v1/transactions/" + transactionId;
        mock.onGet(matcher1).reply(
            200,
            SAMPLE_ETHEREUM_TRANSACTIONS_ON_ACCOUNT,
        );
        const matcher11 = "/api/v1/transactions";
    ...
    it("Should display ETHEREUM TX details with link to contract as entity ID", async () => {
        ...
        const transactionId =
            SAMPLE_ETHEREUM_TRANSACTIONS_ON_CONTRACT.transactions[0]
                .transaction_id;
        const entityId =
            SAMPLE_ETHEREUM_TRANSACTIONS_ON_CONTRACT.transactions[0].entity_id;
        const timestamp =
            SAMPLE_ETHEREUM_TRANSACTIONS_ON_CONTRACT.transactions[0]
                .consensus_timestamp;
    ...
    it("Should display ETHEREUM TX details with link to token as entity ID", async () => {
        ...
        const transaction =
            SAMPLE_ETHEREUM_TRANSACTIONS_ASSOCIATING_TOKEN.transactions[0];
        ...
        const result = {
            ...
            block_hash:
                "0xb020ca98489bf4ceecb650332de36b22b8bfe2cb8d2234d2160e2502b44198a12342ebe427b7ec8078be4afcaadd826b",
        ...
        const matcher4 =
            "/api/v1/contracts/" + result.contract_id + "/results/" + timestamp;
        ...
        const matcher5 =
            "/api/v1/contracts/results/" + result.hash + "/actions?limit=100";
        mock.onGet(matcher5).reply(200, {
            actions: [action],
            links: { next: null },
        });
