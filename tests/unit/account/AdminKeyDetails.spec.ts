...
describe("AdminKeyDetails.vue", () => {
    test("Account view displaying link to key details", async () => {
        ...
        const matcher3 =
            "/api/v1/accounts/" +
            SAMPLE_ACCOUNT_PROTOBUF_KEY.account +
            "/rewards";
        ...
        expect(wrapper.text()).toMatch(
            "Account Account ID:" + SAMPLE_ACCOUNT_PROTOBUF_KEY.account,
        );
        expect(wrapper.get("#keyValue").text()).toBe(
            "Complex Key (6 levels) See details",
        );
    ...
    test("AdminKeyDetails displaying complex (Protobuf) key", async () => {
        ...
        expect(wrapper.text()).toMatch(
            "Admin Key for Account " + SAMPLE_ACCOUNT_PROTOBUF_KEY.account,
        );
    ...
    test("AdminKeyDetails displaying simple (ED25519) key", async () => {
        ...
        expect(wrapper.text()).toMatch(
            "Admin Key for Account " + SAMPLE_ACCOUNT.account,
        );
        ...
        expect(key.text()).toBe(
            "ED25519: aa2f7b3e759f4531ec2e7941afa449e6a6e610efb52adae89e9cd8e9d40ddcbf",
        );
