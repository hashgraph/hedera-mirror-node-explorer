...
describe("NodeDetails.vue", () => {
    it("should display node details", async () => {
        ...
        expect(wrapper.get("#descriptionValue").text()).toBe(
            "Hosted by Hedera | East Coast, USA",
        );
        expect(wrapper.get("#publicKeyValue").text()).toBe(
            "3082 01a2 300d 0609CopyRSA",
        );
        expect(wrapper.get("#fileValue").text()).toBe("0.0.102");
        expect(wrapper.get("#rangeFromValue").text()).toBe(
            "4:10:06.0411 PMJun 6, 2022, UTC",
        );
        expect(wrapper.get("#rangeToValue").text()).toBe("None");
        expect(wrapper.get("#nodeCertHashValue").text()).toBe(
            "d317 df77 a69d 6bbe 1add adf8 6bCopy",
        );
        expect(wrapper.get("#serviceEndpointsValue").text()).toBe(
            "3.211.248.172:502113.211.248.172:5021235.231.208.148:035.231.208.148:5021135.231.208.148:50212",
        );

        expect(wrapper.get("#yearlyRate").text()).toBe(
            "Last Period Reward Rate1%APPROX ANNUAL EQUIVALENT",
        );
        expect(wrapper.get("#consensusStake").text()).toBe(
            "Stake for Consensus6,000,000HBAR",
        );
        expect(wrapper.get("#consensusStakePercent").text()).toBe(
            "25% of total",
        );
        ...
        expect(wrapper.get("#rewarded").text()).toBe(
            "Staked for Reward5,000,000HBAR",
        );
        expect(wrapper.get("#rewardedPercent").text()).toBe("26.32% of total");
        expect(wrapper.get("#notRewarded").text()).toBe(
            "Staked For No Reward1,000,000HBAR",
        );
        expect(wrapper.get("#notRewardedPercent").text()).toBe("20% of total");
        expect(wrapper.get("#stakingPeriod").text()).toBe(
            "Current Staking Period24HOURS",
        );
