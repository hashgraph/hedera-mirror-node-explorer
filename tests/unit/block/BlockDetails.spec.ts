...
describe("BlockDetails.vue", () => {
    ...
    const NORMALIZED_BLOCK_HASH =
        PathParam.parseBlockLoc(BLOCK_HASH)!.toString();
    ...
    it("Should display block details from block number", async () => {
        ...
        expect(wrapper.get("#fromTimestampValue").text()).toBe(
            "6:58:31.3281 AMSep 23, 2022, UTC",
        );
        expect(wrapper.get("#toTimestampValue").text()).toBe(
            "6:58:31.3281 AMSep 23, 2022, UTC",
        );
        expect(wrapper.get("#gasUsedValue").text()).toBe("0");
        expect(wrapper.get("#recordFileNameValue").text()).toBe(
            "2022-09-23T06_58_31.328130742Z.rcd.gz",
        );
        expect(wrapper.get("#blockTransactions").text()).toMatch(
            RegExp("^Block Transactions"),
        );
    ...
    it("Should update when block hash changes", async () => {
        ...
        expect(wrapper.get("#toTimestampValue").text()).toBe(
            "6:58:29.2397 AMSep 23, 2022, UTC",
        );
        expect(wrapper.get("#gasUsedValue").text()).toBe("0");
        expect(wrapper.get("#recordFileNameValue").text()).toBe(
            "2022-09-23T06_58_28.211469425Z.rcd.gz",
        );
        expect(wrapper.get("#blockTransactions").text()).toMatch(
            RegExp("^Block Transactions"),
        );
    ...
    it("Should detect invalid block number", async () => {
        ...
        expect(banner.text()).toBe(
            "Invalid block number or hash: " + INVALID_BLOCK_NUMBER,
        );
    ...
    it("Should detect invalid block hash", async () => {
        ...
        const banner = wrapper.findComponent(NotificationBanner);
        expect(banner.text()).toBe(
            "Invalid block number or hash: " + INVALID_BLOCK_HASH,
        );
    ...
    it("Should detect non-existent block number", async () => {
        ...
        const banner = wrapper.findComponent(NotificationBanner);
        expect(banner.text()).toBe(
            "Block with number " + BLOCK_NUMBER + " was not found",
        );
    ...
    it("Should detect non-existent block hash", async () => {
        ...
        const banner = wrapper.findComponent(NotificationBanner);
        expect(banner.text()).toBe(
            "Block with hash " + NORMALIZED_BLOCK_HASH + " was not found",
        );
