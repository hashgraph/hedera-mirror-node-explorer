...
describe("Staking.vue", () => {
    test("no props", async () => {
        ...
        const TARGET_ACCOUNT = JSON.parse(
            JSON.stringify(SAMPLE_ACCOUNT_STAKING_ACCOUNT),
        );
        ...
        const STAKE_UPDATE_TRANSACTION_ID =
            STAKE_UPDATE_TRANSACTION.transaction_id;

        // Adds test driver to WalletManager
        const testDriver = new WalletDriver_Mock(
            TARGET_ACCOUNT,
            STAKE_UPDATE_TRANSACTION_ID,
        );
        ...
        const matcher8 =
            "/api/v1/accounts/" + testDriver.account.account + "/rewards";
        ...
        expect(
            walletChooser.get(".modal").element.classList.contains("is-active"),
        ).toBeTruthy();
        ...
        expect(wrapper.get("#disconnectWalletButton").text()).toBe(
            "DISCONNECT WALLETMOCK",
        );
        ...
        expect(
            stakingModal.element.classList.contains("is-active"),
        ).toBeTruthy();
        expect(stakingModal.get("#amountStakedValue").text()).toBe(
            "0.31669471$0.07792",
        );
        expect(stakingModal.get("#currentlyStakedToValue").text()).toBe(
            "Account 0.0.5",
        );
        ...
        const stakingRadios = stakingModal.findAll<HTMLInputElement>(
            "input[type='radio']",
        );
        ...
        const newStakeToAccountInput =
            stakingModal.get<HTMLInputElement>("input[type='text']");
        ...
        const confirmChangeStaking = async (expectedText: string) => {
            const changeConfirmDialog =
                stakingDialog.getComponent(ConfirmDialog);
        ...
        await confirmChangeStaking(
            "Change Staking  for account 0.0.730632Do you want to stake to account 0.0.7-bmurp ?FillerCANCELCONFIRM",
        );
        ...
        await waitAndClose(
            "Updating stakingConnecting to Hedera Network using your wallet…Check your wallet for any approval requestCLOSE",
        ...
        expect(stakeToNodeOptions.length).toBe(
            SAMPLE_NETWORK_NODES.nodes.length,
        );
        for (let i = 0; i < 3; i += 1) {
        ...
        // 3.6) Confirms
        await confirmChangeStaking(
            "Change Staking  for account 0.0.730632Do you want to stake to Node 2 - Hosted by Hedera ?FillerCANCELCONFIRM",
        );
        ...
        expect(ndis[0].text()).toBe(
            "Staked toNode 2 - Hosted by Hedera since Nov 11, 2022, 00:00 UTC",
        );
        expect(ndis[1].text()).toBe("My Stake0.31669471HBAR");
        expect(ndis[2].text()).toBe("Pending RewardNone");
        ...
        expect(stakingModal.get("#currentlyStakedToValue").text()).toBe(
            "Node 2 - Hosted by Hedera",
        );
        expect(changeButton.text()).toBe("CHANGE");
        // expect(changeButton.attributes("disabled")).toBeDefined()

        // 4.3) Uncheck "Decline Rewards" box
        const stakingCheckBoxes = stakingModal.findAll<HTMLInputElement>(
            "input[type='checkbox']",
        );
        ...
        // 4.5) Confirms
        await confirmChangeStaking(
            "Change Staking  for account 0.0.730632Do you want to accept rewards?FillerCANCELCONFIRM",
        );
        ...
        await confirm(
            "My Staking  for account 0.0.730632Do you want to stop staking to Node 2 - Hosted by Hedera ?FillerCANCELCONFIRM",
        );

        // 5.3) Waits for progress dialog and closes ...
        await waitAndClose(
            "Stopping stakingCompleting operation…This may take a few secondsCLOSE",
