...
describe("AccountLocParser.ts", () => {
    const SAMPLE_ACCOUNT_ALIAS_HEX = AccountAlias.parse(
        SAMPLE_ACCOUNT.alias,
    )!.toHexString();
    const SAMPLE_ACCOUNT_ADDRESS = makeEthAddressForAccount(
        SAMPLE_ACCOUNT as AccountInfo,
    );
    ...
    test("mount + set/unset account loc + unmount", async () => {
        ...
        expect(parser.createdTimestamp.value).toBe(
            SAMPLE_ACCOUNT.created_timestamp,
        );
        expect(parser.key.value).toStrictEqual(SAMPLE_ACCOUNT.key);
        expect(parser.tokens.value).toStrictEqual(
            SAMPLE_ACCOUNT.balance.tokens,
        );
        expect(parser.stakedNodeId.value).toBeNull();
        expect(parser.stakedAccountId.value).toBeNull();
        expect(parser.stakePeriodStart.value).toBeNull();
    ...
    test("set account loc with unknown account id", async () => {
        ...
        expect(parser.errorNotification.value).toBe(
            "Account with ID 0.0.42 was not found",
        );
    ...
    test("set account loc with unknown account address", async () => {
        ...
        expect(parser.aliasByteString.value).toBeNull();
        expect(parser.errorNotification.value).toBe(
            "Own this account? Activate it by transferring any amount of ℏ or tokens to 0x00…070809.",
        );
    ...
    test("set account loc with unknown alias in base32", async () => {
        ...
        expect(parser.aliasByteString.value).toBeNull();
        expect(parser.errorNotification.value).toBe(
            "Account with alias CIQAAAH4AY2OFK2FL37TSPYEQGPPUJRP4XTKWHD62HKPQX543DTOFFQ was not found",
        );
    ...
    test("set account loc with dummy value", async () => {
        ...
        expect(parser.aliasByteString.value).toBeNull();
        expect(parser.errorNotification.value).toBe(
            "Invalid account ID, address or alias: dummy loc",
        );
        await flushPromises();
        expect(parser.accountLoc.value).toBe(DUMMY_LOC);
        expect(parser.accountInfo.value).toBeNull();
