...
const matcher_transaction_with_timestamp =
    "/api/v1/transactions?timestamp=" +
    SAMPLE_CONTRACT_RESULT_DETAILS.timestamp;
mock.onGet(matcher_transaction_with_timestamp).reply(200, SAMPLE_TRANSACTIONS);

// Block

const BLOCK_HASH = byteToHex(
    hexToByte(SAMPLE_BLOCKSRESPONSE.blocks[0].hash) ?? new Uint8Array(0),
);
...
const BLOCK_HASH_PREFIX = byteToHex(
    hexToByte(SAMPLE_BLOCKSRESPONSE.blocks[0].hash)?.slice(0, 32) ??
        new Uint8Array(0),
);
const matcher_block_with_hash_prefix = "/api/v1/blocks/" + BLOCK_HASH_PREFIX;
mock.onGet(matcher_block_with_hash_prefix).reply(
    200,
    SAMPLE_BLOCKSRESPONSE.blocks[0],
);
...
const matcher_contracts_with_evm_address =
...
const matcher_contracts_with_invalid_evm_address =
    "/api/v1/contracts/" + INVALID_EVM_ADDRESS;
...
describe("SearchRequest.ts", () => {
    ...
    test("account (with alias expressed in hex)", async () => {
        const SAMPLE_ALIAS_HEX = byteToHex(
            base32ToAlias(SAMPLE_ACCOUNT.alias)!,
        );
