...
describe("TransactionGroupByBlockCache", () => {
    test("TransactionGroupByBlockCache", async () => {
        ...
        mock.onGet(matcher2).reply((config: AxiosRequestConfig) => {
            if (
                config.params.timestamp == "lte:" + SAMPLE_BLOCK.timestamp.to &&
        ...
        const transactions =
            await TransactionGroupByBlockCache.instance.lookup(blockNumber);
        await flushPromises();
        expect(transactions).toStrictEqual(
            SAMPLE_PARENT_CHILD_TRANSACTIONS.transactions,
        );
        ...
        const transactions2 =
            await TransactionGroupByBlockCache.instance.lookup(blockNumber);
        await flushPromises();
        expect(transactions2).toStrictEqual(
            SAMPLE_PARENT_CHILD_TRANSACTIONS.transactions,
        );
        ...
        for (const t of SAMPLE_PARENT_CHILD_TRANSACTIONS.transactions) {
            expect(
                TransactionByHashCache.instance.contains(t.transaction_hash),
            ).toBeTruthy();
            expect(
                TransactionByTsCache.instance.contains(t.consensus_timestamp),
            ).toBeTruthy();
        }
    });
});
