...
describe("TableController.ts", () => {
    ...
    test("mount / unmount", async () => {
        ...
        expect(tc.totalRowCount.value).toBe(
    ...
    test("source watch and reload", async () => {
        const scale = ref(1);
        const tc = new TestTableController(
            0,
            50,
            10,
            computed(() => scale.value),
        );
        const currentRoute = tc.router.currentRoute;
    ...
    test("multiple controllers (sharing same router)", async () => {
        ...
        const tc1 = new TestTableController(
            0,
            50,
            10,
            scale,
        ...
        const tc2 = new TestTableController(
            100,
            200,
            10,
            scale,
        ...
        expect(tc1.totalRowCount.value).toBe(
            TestTableController.PRESUMED_ROW_COUNT,
        );
        expect(tc1.rows.value).toStrictEqual([
            49, 48, 47, 46, 45, 44, 43, 42, 41, 40,
        ]);
        ...
        expect(tc2.rows.value).toStrictEqual([
