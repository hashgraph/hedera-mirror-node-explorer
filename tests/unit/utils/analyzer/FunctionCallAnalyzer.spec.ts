...
import {
    ...
    NameTypeValue,
} from "@/utils/analyzer/FunctionCallAnalyzer";
...
describe("FunctionCallAnalyzer.spec.ts", () => {
    test("basic flow", async () => {
        ...
        input.value =
            "0x49146bde000000000000000000000000845b706151aed537b1fd81c1ea4ea03920097abd0000000000000000000000000000000000000000000000000000000002e6ae09";
        output.value =
        ...
        expect(functionCallAnalyzer.signature.value).toBe(
        ...
        input.value =
            "0x618dc65e0000000000000000000000000000000000163b5a70a082310000000000000000000000005fe56763c7633efefe8c2272f19732521a48e300";
        output.value =
            "0x00000000000000000000000000000000000000000000000000003dc604b33217";
        contractId.value = "0.0.359";
        await flushPromises();
        expect(functionCallAnalyzer.functionHash.value).toBeNull();
        expect(functionCallAnalyzer.signature.value).toBeNull();
        ...
        expect(functionCallAnalyzer.errorSignature.value).toBeNull();
        expect(functionCallAnalyzer.errorInputs.value).toStrictEqual([]);
        expect(functionCallAnalyzer.inputDecodingStatus.value).toBe(
            "Decoding Error (data out-of-bounds)",
        );
        expect(functionCallAnalyzer.outputDecodingStatus.value).toBe(
            "Decoding Error (data out-of-bounds)",
        );
        expect(functionCallAnalyzer.errorDecodingStatus.value).toBeNull();
        ...
        expect(functionCallAnalyzer.inputDecodingStatus.value).toBeNull();
        expect(functionCallAnalyzer.outputDecodingStatus.value).toBe(
            "Decoding Error (hex data is odd-length)",
        );
        expect(functionCallAnalyzer.errorDecodingStatus.value).toBeNull();
