...
describe("EthereumAddress", () => {
    ...
    test("Parsing from an invalid address", () => {
        const invalidEvmAddress =
            "0xe6d5514b8de7ef9e5f5c4cc2e8ca0207129deb6500";
    ...
    test("EIP55", () => {
        ...
        expect(EthereumAddress.normalizeEIP55("0x" + random)).toBe(
            "0x" + random,
        );
        expect(
            EthereumAddress.normalizeEIP55("0x" + random.toLowerCase()),
        ).toBe("0x" + random);
        expect(
            EthereumAddress.normalizeEIP55("0x" + random.toUpperCase()),
        ).toBe("0x" + random);

        const normal = "fB6916095ca1df60bB79Ce92cE3Ea74c37c5d359";
        expect(EthereumAddress.normalizeEIP55("0x" + normal)).toBe(
            "0x" + normal,
        );
        expect(
            EthereumAddress.normalizeEIP55("0x" + normal.toLowerCase()),
        ).toBe("0x" + normal);
        expect(
            EthereumAddress.normalizeEIP55("0x" + normal.toUpperCase()),
        ).toBe("0x" + normal);

        const allCaps = "52908400098527886E0F7030069857D2E4169EE7";
        expect(EthereumAddress.normalizeEIP55("0x" + allCaps)).toBe(
            "0x" + allCaps,
        );
        expect(
            EthereumAddress.normalizeEIP55("0x" + allCaps.toLowerCase()),
        ).toBe("0x" + allCaps);
        expect(
            EthereumAddress.normalizeEIP55("0x" + allCaps.toUpperCase()),
        ).toBe("0x" + allCaps);

        const allLower = "de709f2102306220921060314715629080e2fb77";
        expect(EthereumAddress.normalizeEIP55("0x" + allLower)).toBe(
            "0x" + allLower,
        );
        expect(
            EthereumAddress.normalizeEIP55("0x" + allLower.toLowerCase()),
        ).toBe("0x" + allLower);
        expect(
            EthereumAddress.normalizeEIP55("0x" + allLower.toUpperCase()),
        ).toBe("0x" + allLower);
    });
});
