...
describe("TransactionLocParser.ts", () => {
    ...
    test("mount + set/unset transaction loc + unmount", async () => {
        ...
        expect(parser.transactionLoc.value).toBe(
            SAMPLE_TRANSACTION.consensus_timestamp,
        );
        expect(parser.transaction.value).toBeNull();
        expect(parser.transactionId.value).toBeNull();
        ...
        expect(parser.transactionId.value).toBe(
            SAMPLE_TRANSACTION.transaction_id,
        );
        expect(parser.consensusTimestamp.value).toBe(
            SAMPLE_TRANSACTION.consensus_timestamp,
        );
        expect(parser.transactionHash.value).toBe(
            SAMPLE_TRANSACTION.transaction_hash,
        );
    ...
    test("set transaction loc with transaction hash", async () => {
        ...
        const transactionHash = TransactionHash.parseBase64(
            SAMPLE_TRANSACTION.transaction_hash,
        )!.toString();
    ...
    test("set transaction loc with ethereum hash", async () => {
        ...
        const matcher1 =
            "/api/v1/contracts/results/" + SAMPLE_CONTRACT_RESULT_DETAILS.hash;
        ...
        mock.onGet(matcher2).reply((config: AxiosRequestConfig) => {
            if (
                config.params.timestamp ==
                SAMPLE_CONTRACT_CALL_TRANSACTION.consensus_timestamp
            ) {
                return [
                    200,
                    { transactions: [SAMPLE_CONTRACT_CALL_TRANSACTION] },
                ];
            } else {
                return [404];
            }
        ...
        expect(parser.transactionLoc.value).toBe(
            SAMPLE_CONTRACT_RESULT_DETAILS.hash,
        );
        expect(parser.transaction.value).toBeNull();
        expect(parser.transactionId.value).toBeNull();
        expect(parser.consensusTimestamp.value).toBeNull();
        ...
        expect(parser.transaction.value).toStrictEqual(
            SAMPLE_CONTRACT_CALL_TRANSACTION,
        );
        expect(parser.transactionId.value).toBe(
            SAMPLE_CONTRACT_CALL_TRANSACTION.transaction_id,
        );
        expect(parser.consensusTimestamp.value).toBe(
            SAMPLE_CONTRACT_CALL_TRANSACTION.consensus_timestamp,
        );
        expect(parser.transactionHash.value).toBe(
            SAMPLE_CONTRACT_CALL_TRANSACTION.transaction_hash,
        );
        expect(parser.errorNotification.value).toBe(null);
    ...
    test("set transaction loc with transaction id (single transaction with nonce 0)", async () => {
        ...
        const PARENT_TRANSACTION =
            SAMPLE_PARENT_CHILD_TRANSACTIONS.transactions[0];
        const matcher1 =
            "/api/v1/transactions/" + PARENT_TRANSACTION.transaction_id;
        mock.onGet(matcher1).reply((config: AxiosRequestConfig) => {
        ...
        expect(parser.consensusTimestamp.value).toBe(
            PARENT_TRANSACTION.consensus_timestamp,
        );
        expect(parser.transactionHash.value).toBe(
            PARENT_TRANSACTION.transaction_hash,
        );
        expect(parser.errorNotification.value).toBe(null);
    ...
    test("set transaction loc with transaction id (two transactions with nonce 0)", async () => {
        ...
        const SCHEDULING_TRANSACTION =
            SAMPLE_SCHEDULING_SCHEDULED_TRANSACTIONS.transactions[0];
        const SCHEDULED_TRANSACTION =
            SAMPLE_SCHEDULING_SCHEDULED_TRANSACTIONS.transactions[1];
        expect(SCHEDULING_TRANSACTION.transaction_id).toBe(
            SCHEDULED_TRANSACTION.transaction_id,
        );
        ...
        expect(parser.consensusTimestamp.value).toBe(
            SCHEDULED_TRANSACTION.consensus_timestamp,
        );
        expect(parser.transactionHash.value).toBe(
            SCHEDULED_TRANSACTION.transaction_hash,
        );
        expect(parser.errorNotification.value).toBe(null);
    ...
    test("set transaction loc with unknown consensus timestamp", async () => {
        ...
        expect(parser.consensusTimestamp.value).toBeNull();
        expect(parser.transactionHash.value).toBeNull();
        expect(parser.errorNotification.value).toBe(
            "Transaction with timestamp 7777777777.888888888 was not found",
        );
    ...
    test("set transaction loc with unknown transaction hash", async () => {
        ...
        const UNKNOWN_HASH =
            "0x001122334455667788990011223344556677889900112233445566778899001122334455667788990011223344556677";
        ...
        expect(parser.transactionHash.value).toBeNull();
        expect(parser.errorNotification.value).toBe(
            "Transaction with hash 001122334455667788990011223344556677889900112233445566778899001122334455667788990011223344556677 was not found",
        );
    ...
    test("set transaction loc with unknown ethereum hash", async () => {
        ...
        const UNKNOWN_ETH_HASH =
            "0x0011223344556677001122334455667700112233445566770011223344556677";
        ...
        expect(parser.transactionHash.value).toBeNull();
        expect(parser.errorNotification.value).toBe(
            "Transaction with ethereum hash 0x0011223344556677001122334455667700112233445566770011223344556677 was not found",
        );
    ...
    test("set transaction loc with dummy value", async () => {
        ...
        expect(parser.transactionHash.value).toBeNull();
        expect(parser.errorNotification.value).toBe(
            "Invalid transaction id, timestamp or hash: " + DUMMY_LOC,
        );
        await flushPromises();
        expect(parser.transactionLoc.value).toBe(DUMMY_LOC);
        expect(parser.transaction.value).toBeNull();
