...
describe("ContractDetails.vue", () => {
    it("Should display contract details", async () => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp("^Contract Contract ID:" + SAMPLE_CONTRACT.contract_id),
        );
        expect(wrapper.get("#balanceValue").text()).toBe("2.00000000$0.49207");
        expect(wrapper.get("#keyValue").text()).toBe(
            "4210 5082 0e14 85ac dd59 7260 88e0 e4a2 130e bbbb 7000 9f64 0ad9 5c78 dd5a 7b38CopyED25519",
        );
        ...
        expect(wrapper.get("#validFromValue").text()).toBe(
        ...
        expect(wrapper.get("#evmAddress").text()).toBe(
            "EVM Address:0x00000000000000000000000000000000000b70cfCopy",
        );
        expect(wrapper.get("#code").text()).toBe("Runtime Bytecode");
        expect(wrapper.get("#solcVersion").text()).toBe(
            "Compiler Version0.8.4",
        );
        expect(wrapper.get("#ipfsHash").text()).toBe(
            "IPFS HashQmap1zNn5JRVVoLFDAKbah7jZyVJAvjqq7f8oUExesSiWT",
        );
    ...
    it("Should display recent contract calls table in contract details", async () => {
        ...
        expect(resultTable.find("thead").text()).toBe(
            "Time From Error Message Transfer Amount",
        );
        ...
        expect(cells[1].text()).toBe(
            "0x00000000000000000000000000000000000004ec(0.0.1260)",
        );
    ...
    it("Should update when contract id changes", async () => {
        ...
        let matcher5 =
            "/api/v1/contracts/" + contract1.contract_id + "/results";
        mock.onGet(matcher5).reply(200, SAMPLE_CONTRACT_RESULTS);
        ...
        expect(wrapper.text()).toMatch(
            RegExp("Contract Contract ID:" + SAMPLE_CONTRACT_DUDE.contract_id),
        );
    ...
    it.skip("Should display notification of grace period", async () => {
        ...
        const matcher5 =
            "/api/v1/contracts/" + contract.contract_id + "/results";
        mock.onGet(matcher5).reply(200, SAMPLE_CONTRACT_RESULTS);
        ...
        expect(wrapper.text()).toMatch(
            RegExp("^Contract " + SAMPLE_CONTRACT_DUDE.contract_id),
        );

        const banner = wrapper.findComponent(NotificationBanner);
        expect(banner.exists()).toBe(true);
        expect(banner.text()).toBe(
            "Contract has expired and is in grace period",
        );
    ...
    it("Should display notification of deleted contract", async () => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp("^Contract Contract ID:" + contract.contract_id),
        );
    ...
    it("Should detect invalid contract ID", async () => {
        ...
        expect(wrapper.get("#notificationBanner").text()).toBe(
            "Invalid contract ID: " + invalidContractId,
        );
    ...
    it("Should display swarm hash", async () => {
        ...
        expect(wrapper.get("#swarmHashValue").text()).toBe(
            "0x25b12311dff4c2d38251fa91e465b5df31fca9f6c32e034ba551935d652b757a",
        );
    ...
    it("Should display contract verification link and properties", async () => {
        ...
        expect(wrapper.text()).toMatch(
            RegExp("^Contract Contract ID:" + contractId),
        );
        ...
        expect(cards[1].get("#verificationStatusValue").text()).toBe(
            "Not yet verified",
        );
