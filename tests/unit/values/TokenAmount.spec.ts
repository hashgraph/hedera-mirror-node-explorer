...
import {
    SAMPLE_TOKEN,
    SAMPLE_TOKEN_DUDE,
    SAMPLE_TOKEN_WITH_LARGE_DECIMAL_COUNT,
} from "../Mocks";
...
const matcher3 =
    "/api/v1/tokens/" + SAMPLE_TOKEN_WITH_LARGE_DECIMAL_COUNT.token_id;
...
describe("TokenAmount.vue", () => {
    ...
    it("should detect too large decimal count", async () => {
        ...
        expect(wrapper.get("a").attributes("href")).toMatch(
            RegExp(
                "/token/" +
                    SAMPLE_TOKEN_WITH_LARGE_DECIMAL_COUNT.token_id +
                    "$",
            ),
        );
        expect(wrapper.get(".h-is-extra-text").text()).toBe(
            SAMPLE_TOKEN_WITH_LARGE_DECIMAL_COUNT.symbol,
        );
        expect(wrapper.text()).toBe(
            "?TTOK0This token amount cannot be displayed because the number of decimals (75) of the token is too large",
        );
